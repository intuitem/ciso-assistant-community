...
<ul data-testid="domain-analytics-treeview">
	<li>
		<div class="mb-1">
			...
			{#if children}
				<div class="flex items-center w-3 me-4 text-center cursor-pointer transition-transform duration-200 {!expanded ? '-rotate-90' : ''}" onclick={toggleExpansion} data-testid="treeview-expand-arrow-elem">
				...
				{/if}
				{#if uuid && viewable}
					<a {href} class="text-xl font-semibold hover:text-primary-500 block" data-testid="treeview-label-text-elem">{name}</a>
				{:else}
