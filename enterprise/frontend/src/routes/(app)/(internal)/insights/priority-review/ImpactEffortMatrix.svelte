<script lang="ts">
	...
	function getCellStyle(rowIndex: number, colIndex: number) {
		...
		if (impact >= 3 && effort <= 2)
			return 'bg-green-200 hover:bg-green-300 dark:bg-green-500/20 dark:hover:bg-green-500/30';
		if (impact >= 3 && effort >= 3)
			return 'bg-yellow-200 hover:bg-yellow-300 dark:bg-yellow-500/20 dark:hover:bg-yellow-500/30';
		if (impact <= 2 && effort >= 4)
			return 'bg-red-200 hover:bg-red-300 dark:bg-red-500/20 dark:hover:bg-red-500/30';
		return 'bg-surface-100-900 hover:bg-surface-200-800';
	}
...
<main class="grid grid-cols-6">
	<div class="w-full max-w-screen-lg mx-auto p-4 col-span-4">
		<div class="grid grid-cols-6 gap-1 mb-6">
			...
			<div class="col-span-5 row-span-5 border-2 border-surface-300-700 grid grid-cols-5 gap-1 p-1">
				{#each data as row, rowIndex}
					{#each row as col, colIndex}
						<button
							class="aspect-square flex flex-col items-center justify-center border border-surface-300-700 rounded text-xs p-1 transition-colors {getCellStyle(
								rowIndex,
								colIndex
	...
	<div class="mb-4 p-4 rounded">
		...
		<div class="flex flex-wrap gap-4 text-sm">
			<div class="flex items-center gap-2">
				<div class="w-4 h-4 bg-green-200 dark:bg-green-500/20 border"></div>
				<span>{m.quickWins()}</span>
			</div>
			<div class="flex items-center gap-2">
				<div class="w-4 h-4 bg-yellow-200 dark:bg-yellow-500/20 border"></div>
				<span>{m.majorProjects()}</span>
			</div>
			<div class="flex items-center gap-2">
				<div class="w-4 h-4 bg-surface-100-900 border"></div>
				<span>{m.fillIns()}</span>
			</div>
			<div class="flex items-center gap-2">
				<div class="w-4 h-4 bg-red-200 dark:bg-red-500/20 border"></div>
...
<div class="px-4 text-xs text-surface-600-400">
