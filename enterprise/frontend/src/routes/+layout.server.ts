...
export const load: LayoutServerLoad = async ({ fetch, locals }) => {
  ...
  if (!locals.globalSettings) {
    const _settings = await fetch("/settings/client-settings").then((res) =>
      res.json(),
    );
    clientSettings = { name: "clientSettings", settings: _settings };
  } else clientSettings = locals.globalSettings;
  const licenseStatus = await fetch(`${BASE_API_URL}/license-status/`).then(
    (res) => res.json(),
  );
  if (
    !locals.user &&
    clientSettings.settings.show_images_unauthenticated !== true
  ) {
  ...
  const LICENSE_EXPIRATION_NOTIFY_DAYS = Object.hasOwn(
    env,
    "PUBLIC_LICENSE_EXPIRATION_NOTIFY_DAYS",
  )
