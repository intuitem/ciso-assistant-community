...
class TestUserGroups:
    ...
    def test_group_permissions(self, test):
        ...
        user_permissions = RoleAssignment.get_permissions(User.objects.get(email=TEST_USER_EMAIL))
        for perm in GROUPS_PERMISSIONS[test.user_group]['perms']:
            assert perm in user_permissions.keys(), f"Permission {perm} not found in user permissions (group: {test.user_group})"
