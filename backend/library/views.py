...
from core.models import StoredLibrary, LoadedLibrary, LibraryUpdater, RiskMatrix
...
class RiskMatrixDraftViewSet(BaseModelViewSet):
    """ViewSet for managing risk matrix drafts in the visual editor."""

    serializers_module = "library.serializers"
    ...
    def publish(self, request, pk=None):
        """Publish a draft as a StoredLibrary, then auto-load it."""
        from library.models import RiskMatrixDraft

        draft = self.get_object()

        if draft.status == RiskMatrixDraft.Status.PUBLISHED:
            return Response(
                {"error": "This draft has already been published."},
                status=HTTP_400_BAD_REQUEST,
            )

        json_def = draft.json_definition
        errors = self._validate_json_definition(json_def)
        if errors:
            return Response({"errors": errors}, status=HTTP_400_BAD_REQUEST)

        # Build a ref_id from the draft name
        ref_id = draft.name.lower().replace(" ", "-").replace("_", "-")
        urn = f"urn:intuitem:risk:library:risk-matrix-{ref_id}"

        # Build library YAML-equivalent dict
        library_data = {
            "urn": urn,
            "locale": draft.locale,
            "ref_id": ref_id,
            ...
            "provider": draft.provider or "custom",
            "packager": "custom",
            "objects": {
                "risk_matrix": [
                    {
                        "urn": f"urn:intuitem:risk:matrix:{ref_id}",
                        "ref_id": ref_id,
                        "name": draft.name,
                        "description": draft.description or "",
        ...
        library_content = yaml.dump(library_data, allow_unicode=True).encode("utf-8")

        try:
            stored_lib, error = StoredLibrary.store_library_content(library_content)
        ...
        try:
            importer = LibraryImporter(stored_lib)
            error = importer.import_library()
            if error:
                return Response(
                    {"error": error},
                    status=HTTP_400_BAD_REQUEST,
            ...
            logger.error("Failed to auto-load published matrix library", error=str(e))
            return Response(
                {"error": f"Library stored but auto-load failed: {str(e)}"},
                status=HTTP_500_INTERNAL_SERVER_ERROR,
            )

        draft.status = RiskMatrixDraft.Status.PUBLISHED
        draft.save()

        return Response(
            {
                "status": "success",
                "stored_library_id": str(stored_lib.id),
            },
            status=HTTP_201_CREATED,
        )

    @action(detail=False, methods=["post"], url_path="clone-from-matrix")
    def clone_from_matrix(self, request):
        """Create a new draft by cloning an existing loaded RiskMatrix."""
        import copy

        from library.models import RiskMatrixDraft

        source_matrix_id = request.data.get("source_matrix_id")
        if not source_matrix_id:
            return Response(
                {"error": "source_matrix_id is required."},
                status=HTTP_400_BAD_REQUEST,
            )

        try:
            source = RiskMatrix.objects.get(id=source_matrix_id)
        except RiskMatrix.DoesNotExist:
            return Response(
                {"error": "Source matrix not found."},
                status=HTTP_404_NOT_FOUND,
            )

        folder_id = request.data.get("folder", str(Folder.get_root_folder().id))

        draft = RiskMatrixDraft.objects.create(
            name=f"{source.name} (copy)",
            description=source.description,
            folder_id=folder_id,
            json_definition=copy.deepcopy(source.json_definition),
        ...
        from library.serializers import RiskMatrixDraftReadSerializer

        serializer = RiskMatrixDraftReadSerializer(draft)
