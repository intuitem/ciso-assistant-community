...
if [ -z "$DJANGO_SECRET_KEY" ]; then
  ...
  if [ ! -f db/django_secret_key ]; then
    python3 - <<'PY' > db/django_secret_key
import secrets
print(secrets.token_hex(32))
PY
    echo "generating initial Django secret key"
  fi
  export DJANGO_SECRET_KEY="$(cat db/django_secret_key)"
  echo "Django secret key read from file"
fi

while ! poetry run python manage.py showmigrations iam >/dev/null 2>&1; do
...
exec poetry run gunicorn --chdir ciso_assistant \
