// Risk assessment report — English.
...
#let axis-title = (
  probabilityISO: "Likelihood",
  probabilityEBIOS: "Likelihood",
  impactISO: "Impact",
  impactEBIOS: "Severity",
)
#let axis-name(kind) = axis-title.at(kind + d.label_standard)
#let qualification-label = if d.use_risk_category_label { "Risk categories" } else { "Qualifications" }
...
#set page(
  ...
  header: context {
    if counter(page).get().first() > 1 {
      ...
      grid(columns: (1fr, auto), [Risk assessment], align(right)[#ra.name])
...
#page(header: none, footer: none)[
  ...
  #align(center + horizon)[
    ...
    #v(0.4em)
    #text(14pt, fill: muted)[Risk assessment — #ra.version]
    #if ra.description != "" [
      #v(0.6em)
    ...
    #block(width: 75%)[
      ...
      #grid(
        ...
        column-gutter: 12pt,
        [*Domain*], [#ra.folder],
        [*Perimeter*], [#ra.perimeter],
        [*Risk matrix*], [#ra.matrix],
        [*Status*], [#assessment-status-label.at(ra.status_key, default: "-")],
        [*Authors*], [#ra.authors],
        [*Reviewers*], [#ra.reviewers],
        [*ETA / Due*], [#ra.eta — #ra.due_date],
      )
      #v(1em)
      #block(width: 100%)[
...
#table(
  ...
  table.header(
    ...
    [*Name*],
    [*Description*],
    ..if with-inherent { ([*Inherent*],) } else { () },
    [*Current*],
    [*Residual*],
    [*Treatment*],
  ),
...
#text(fill: muted)[#d.scenarios.len() scenarios.]
#v(0.6em)
    ...
    #grid(
      ...
      ..(
        if scenario.description != "" { row("Description", [#scenario.description]) } else { () }
          ...
          + if scenario.assets.len() > 0 { row("Assets", [#scenario.assets.join(", ")]) } else { () }
          + if scenario.threats.len() > 0 { row("Threats", [#scenario.threats.join(", ")]) } else { () }
          + if "inherent" in scenario { row("Inherent level", level-badge(scenario.inherent)) } else { () }
          + if scenario.existing_controls.len() > 0 { row("Existing controls", [#scenario.existing_controls.join(", ")]) } else { () }
          + row("Current level", level-badge(scenario.current))
          + if scenario.controls.len() > 0 { row("Extra controls", [#scenario.controls.join(", ")]) } else { () }
          + row("Residual level", level-badge(scenario.residual))
          + row("Strength of knowledge", [#scenario.strength_of_knowledge])
          + row("Treatment", [#treatment-label.at(scenario.treatment_key, default: scenario.treatment_key)])
