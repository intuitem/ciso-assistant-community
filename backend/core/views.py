...
class TaskNodeViewSet(BaseModelViewSet):
    ...
    def _generate_occurrences(self, template, start_date, end_date):
        ...
        if base_date < start_date:
            while current_date < start_date:
                next_date = self._calculate_next_occurrence(template, current_date)
        ...
        while current_date and current_date <= end_date:
            ...
            if self._date_matches_schedule(template, current_date):
                occurrences.append(self._create_task_dict(template, current_date, occurrence_count))
                occurrence_count += 1
            
            # Calculate next date
            current_date = self._calculate_next_occurrence(template, current_date)
        
        return occurrences

    def _date_matches_schedule(self, template, date_to_check):
    ...
    def _calculate_next_occurrence(self, template, base_date):
        ...
        if frequency == 'DAILY':
            ...
            if self._date_matches_schedule(template, base_date):
                ...
                while same_month_date.month == base_date.month:
                    if self._date_matches_schedule(template, same_month_date):
            ...
            if weeks_of_month and days_of_week:
                ...
                return self._calculate_next_occurrence(
                    template, 
