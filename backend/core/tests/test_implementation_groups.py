...
@pytest.mark.django_db
class TestComplianceAssessmentProgress:
    def test_progress_counts_score_only_requirements_as_assessed(
        self, dynamic_framework_setup
    ):
        d = dynamic_framework_setup
        d["ra"].result = RequirementAssessment.Result.NOT_ASSESSED
        d["ra"].score = 10
        d["ra"].save(update_fields=["result", "score"])

        second_requirement = RequirementNode.objects.create(
            framework=d["framework"],
            urn="urn:test:ig:req:002",
            ref_id="IG-REQ-2",
            assessable=True,
            folder=d["folder"],
            is_published=True,
        ...
        RequirementAssessment.objects.create(
            compliance_assessment=d["ca"],
            requirement=second_requirement,
            folder=d["folder"],
            result=RequirementAssessment.Result.NOT_ASSESSED,
        )

        # One assessed requirement out of two assessable ones -> 50%.
        assert d["ca"].progress == 50

    def test_progress_filters_selected_implementation_groups_lightweight(
        self, dynamic_framework_setup
    ):
        d = dynamic_framework_setup
        d["requirement_node"].implementation_groups = ["base"]
        d["requirement_node"].save(update_fields=["implementation_groups"])
        d["ra"].result = RequirementAssessment.Result.NOT_ASSESSED
        d["ra"].score = None
        d["ra"].save(update_fields=["result", "score"])

        advanced_requirement = RequirementNode.objects.create(
            framework=d["framework"],
            urn="urn:test:ig:req:003",
            ref_id="IG-REQ-3",
            assessable=True,
            implementation_groups=["advanced"],
            folder=d["folder"],
            is_published=True,
        )
        RequirementAssessment.objects.create(
            compliance_assessment=d["ca"],
            requirement=advanced_requirement,
            folder=d["folder"],
            result=RequirementAssessment.Result.COMPLIANT,
        )

        # Only the "base" requirement is in scope, and it is still untouched.
        d["ca"].selected_implementation_groups = ["base"]
        d["ca"].save()
        d["ca"].refresh_from_db()

        assert d["ca"].progress == 0
