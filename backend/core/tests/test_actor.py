...
class TestActorSync:
    ...
    def test_user_actor_lifecycle(self):
        ...
        user = User.objects.create_user(
            email="test_actor_user@example.com", password="password"
        )
    ...
    def test_team_actor_lifecycle(self):
        ...
        leader = User.objects.create_user(
            email="team_leader@example.com", password="password"
        )
    ...
    def test_user_bulk_create_actor_sync(self):
        ...
        """
        users = [User(email=f"bulk_user_{i}@example.com") for i in range(5)]
    ...
    def test_team_bulk_create_actor_sync(self):
        ...
        """
        leader = User.objects.create_user(
            email="bulk_team_leader@example.com", password="password"
        )

        teams = [Team(name=f"Bulk Team {i}", leader=leader) for i in range(5)]
        ...
        assert Team.objects.filter(name__startswith="Bulk Team").count() == 5
        assert Actor.objects.filter(team__name__startswith="Bulk Team").count() == 5
    ...
    def test_entity_bulk_create_actor_sync(self):
        ...
        """
        entities = [Entity(name=f"Bulk Entity {i}") for i in range(5)]
    ...
    def test_actor_uniqueness_constraint(self):
        ...
        """
        user = User.objects.create_user(
            email="constraint_user@example.com", password="password"
        )
        ...
        with pytest.raises(
            Exception
        ):  # generic exception as it could be IntegrityError or ValidationError
