...
def resolve_visibility_from_overrides(overrides, field_name):
    ...
    """
    # is_score_overridden is a companion of score: it only makes sense when the
    # score is shown, and must be readable/writable under exactly the same
    # conditions. Resolve it against score's visibility (including an explicit
    # per-CA override) rather than as an independent field, otherwise it would
    # fall back to a HIDDEN default and get stripped even when score is visible.
