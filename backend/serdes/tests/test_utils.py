...
from core.models import (
    Folder, Asset, Perimeter, RiskAssessment, RiskScenario, AppliedControl,
    ComplianceAssessment, Framework, Threat, StoredLibrary, RiskMatrix
)
...
def risk_matrix_fixture():
    ...
    assert library is not None
    library.load()
    return RiskMatrix.objects.get(urn="urn:intuitem:risk:matrix:critical_risk_matrix_5x5")
...
class TestObjectSorting:
    ...
    def test_sort_objects_multiple_roots(self):
        ...
        assert sorted_objs.index({"id": 1, "fields": {"parent": []}}) < sorted_objs.index({"id": 3, "fields": {"parent": [1]}})
        assert sorted_objs.index({"id": 2, "fields": {"parent": []}}) < sorted_objs.index({"id": 4, "fields": {"parent": [2]}})
...
class TestDomainExport:
    ...
    def test_get_domain_export_objects_basic(self, complex_domain_structure):
        ...
        assert complex_domain_structure["risk_assessment"] in export_data["riskassessment"]
    ...
    @pytest.mark.django_db
    def test_get_domain_export_objects_subfolder_content(self, complex_domain_structure):
        """Test export of objects in subfolders."""
        root = complex_domain_structure["root"]
        subfolder = complex_domain_structure["subfolder"]
        
        subfolder_asset = Asset.objects.create(
            name="Subfolder Asset",
            folder=subfolder
        )
    ...
    def test_get_domain_export_objects_compliance(self, complex_domain_structure):
        ...
        assert complex_domain_structure["compliance_assessment"] in export_data["complianceassessment"]
    ...
    def test_get_domain_export_objects_cross_references(self, complex_domain_structure):
        ...
        assert risk_scenario in export_data["riskscenario"] and threat in export_data["riskscenario"].first().threats.all()