...
class EntitySerializersTestCase(TestCase):
    ...
    def test_entity_write_serializer(self):
        ...
        with patch('iam.models.RoleAssignment.is_access_allowed', return_value=True):
            serializer = EntityWriteSerializer(data=new_entity_data, context={"request": mock_request})
...
class EntityAssessmentSerializersTestCase(TestCase):
    ...
    @patch('tprm.serializers.EntityAssessmentWriteSerializer._assign_third_party_respondents')
    def test_entity_assessment_write_serializer_with_audit(self, mock_assign_third_party, mock_compliance_assessment, mock_folder_create, mock_audit_create):
        ...
        with patch('iam.models.RoleAssignment.is_access_allowed', return_value=True):
            serializer = EntityAssessmentWriteSerializer(data=data, context={"request": MagicMock()})
            self.assertTrue(serializer.is_valid())
            serializer.save()
        
        mock_audit_create.assert_called_once()
        self.assertEqual(mock_audit_create.call_args[1]['name'], data['name'])
        self.assertEqual(mock_audit_create.call_args[1]['framework'].id, self.framework.id)
        self.assertEqual(mock_audit_create.call_args[1]['perimeter'], self.perimeter)
        self.assertEqual(mock_audit_create.call_args[1]['selected_implementation_groups'], data['selected_implementation_groups'])

        
    @patch('iam.models.RoleAssignment.is_access_allowed', return_value=True)
    def test_entity_assessment_write_serializer_without_framework(self, mock_is_access_allowed):
    ...
    def test_entity_assessment_write_update(self, mock_assign):
        ...
        new_rep = User.objects.create_user(email="newrep@example.com", password="password")
        
        data = {
            "name": "Updated Assessment",
            "representatives": [new_rep.id]
        }
        
        serializer = EntityAssessmentWriteSerializer(self.assessment, data=data, partial=True)
        self.assertTrue(serializer.is_valid())
...
class RepresentativeSerializersTestCase(TestCase):
    ...
    def setUp(self):
        self.folder = Folder.objects.create(name="Test Folder")
        self.entity = Entity.objects.create(name="Test Entity", folder=self.folder)
        self.user = User.objects.create_user(email="existing@example.com", password="password")
    ...
    def test_representative_write_serializer_create_user(self, mock_create_or_update):
        ...
        with patch('iam.models.RoleAssignment.is_access_allowed', return_value=True):
            serializer = RepresentativeWriteSerializer(data=data, context={"request": MagicMock()})
            self.assertTrue(serializer.is_valid())
            serializer.save()
    ...
    def test_representative_write_serializer_existing_user(self, mock_filter, mock_is_access_allowed):
    ...
    @patch('iam.models.User.objects.filter')
    def test_representative_write_serializer_error_handling(self, mock_filter, mock_create_user, mock_is_access_allowed):
...
class SolutionSerializersTestCase(TestCase):
    ...
    def setUp(self):
        self.folder = Folder.objects.create(name="Test Folder")
        self.provider_entity = Entity.objects.create(name="Provider Entity", folder=self.folder)
        self.recipient_entity = Entity.objects.create(name="Recipient Entity", folder=self.folder)
    ...
    def test_solution_write_serializer(self, mock_is_access_allowed):
        ...
        serializer = SolutionWriteSerializer(data=new_solution_data, context={"request": MagicMock()})
        self.assertTrue(serializer.is_valid())
