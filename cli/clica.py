...
def init_config():
    ...
    template_data = {
        ...
        "credentials": {
            ...
            "password": "", 
            "token": ""  # PAT token can be set here directly
        },
    }
    if click.confirm(
        ...
        with open(CLICA_CONFG_PATH, "w") as yfile:
            ...
            print(
                f"Config file is available at {CLICA_CONFG_PATH}. Please update it with your credentials or PAT token."
            )
...
try:
    ...
    # Get PAT token from config file
    CONFIG_TOKEN = cli_cfg["credentials"].get("token", "")
except KeyError:
    print(
...
def check_auth():
    """Check for authentication token, prioritizing PAT token from config"""
    # 1. First check if a PAT token is configured in the config file
    if CONFIG_TOKEN:
        print("Using PAT token from config file", file=sys.stderr)
        return CONFIG_TOKEN
    
    # 2. Otherwise, check if there's a temporary token generated by auth
    if Path(".tmp.yaml").exists():
        with open(".tmp.yaml", "r") as yfile:
            auth_data = yaml.safe_load(yfile)
            print("Using temporary token from previous authentication", file=sys.stderr)
            return auth_data.get("token")
    
    # 3. No token found
    print("No token found. Either set a PAT token in config file or run 'auth' command.", file=sys.stderr)
    return None
...
def auth(email, password):
    """Authenticate to get a temp token (config file or params). Pass the email and password or set them on the config file. Not needed if using PAT token."""
    
    # If a PAT token is already configured, inform the user
    if CONFIG_TOKEN:
        print("PAT token is already configured in config file. Authentication not needed.", file=sys.stderr)
        return
...
def ids_map(model, folder=None):
    if not TOKEN:
        print("No authentication token available. Please set PAT token in config or run auth command.", file=sys.stderr)
        sys.exit(1)
