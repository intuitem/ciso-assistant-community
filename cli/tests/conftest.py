...
def sample_matrices_response():
    ...
    return {
        "Global": {"4x4 risk matrix from EBIOS-RM": 1, "5x5 standard matrix": 2},
        "EBIOS-RM matrix": 2,
        "Custom matrix": 3,
    }
    ...
    def _create_csv(data, filename_suffix="test"):
        if isinstance(data, pd.DataFrame):
            with tempfile.NamedTemporaryFile(
                mode="w", suffix=f"_{filename_suffix}.csv", delete=False
            ) as f:
...
def sample_ids_map_response():
    ...
    return {
        "Global": {"folder1": 1, "folder2": 2},
        "Project1": {"item1": 10, "item2": 20},
    }
