...
class TestAuthentication:
    ...
    def test_token_validation_success(
        self, mock_requests_success, mock_token, auth_headers
    ):
        """
    ...
    def test_api_url_configuration(self, mock_token):
        ...
        test_cases = [
            (
                ...
                "folders",
                "http://localhost:8000/api/folders/ids/",
            ),
            (
                "https://ciso.example.com/api",
                "perimeters",
                "https://ciso.example.com/api/perimeters/ids/",
            ),
            (
                "http://192.168.1.100:8080/api",
                "risk-matrices",
                "http://192.168.1.100:8080/api/risk-matrices/ids/",
            ),
        ]
    ...
    def test_get_folders_success(self, mock_token):
        ...
        with patch("clica.TOKEN", mock_token):
            with patch("requests.get") as mock_get:
                ...
                mock_response.status_code = 200
                mock_response.json.return_value = {
                    "folders": {"Global": 1, "Project1": 2, "BU 1": 3}
                }
                mock_get.return_value = mock_response
...
class TestUtilityFunctions:
    ...
    def test_batch_create_success(self, mock_post):
        ...
        for i, item in enumerate(items):
            ...
            assert call_args[1]["json"] == {"folder": folder_id, "name": item}
...
class TestImportRiskAssessment:
    ...
    def test_import_risk_assessment_complete_workflow(
        self, sample_csv_content, sample_matrix_definition, mock_ids_responses
    ):
        """
...
R.1;Test Risk"""
    ...
    def test_import_risk_assessment_matrix_mismatch(
        self, sample_csv_content, mock_ids_responses
    ):
        """
        ...
        try:
            with (
                ...
                mock_print.assert_any_call(
                    "Matrix doesn't match the labels used on your input file"
                )
    ...
    def test_import_risk_assessment_auth_failure(self, sample_csv_content):
        ...
        try:
            with patch("clica.TOKEN", ""), patch("builtins.print") as mock_print:
                result = runner.invoke(
                    import_risk_assessment,
                    [
                        "--file",
    ...
    def test_import_risk_assessment_api_error(
        self, sample_csv_content, mock_ids_responses
    ):
        """
...
class TestImportAssets:
    ...
    def test_import_assets_success(
        self, mock_confirm, mock_read_csv, mock_get_folders, mock_post
    ):
        ...
        mock_df = pd.DataFrame(
            {"name": ["Server-01", "Database-01"], "type": ["Primary", "Support"]}
        )
        ...
        try:
            ...
            mock_confirm.assert_called_once_with(
                "I'm about to create 2 assets. Are you sure?"
            )
    ...
    @patch("click.confirm")
    @patch("clica.TOKEN", "test-token-12345")
    def test_import_assets_user_cancellation(
        self, mock_confirm, mock_read_csv, mock_get_folders
    ):
        ...
        # Mock CSV data
        mock_df = pd.DataFrame({"name": ["Server-01"], "type": ["Primary"]})
        mock_read_csv.return_value = mock_df
        ...
        try:
            ...
            with patch("clica.requests.post") as mock_post:
                ...
                assert result.exit_code == 0
                mock_confirm.assert_called_once_with(
                    "I'm about to create 1 assets. Are you sure?"
                )
    ...
    @patch("click.confirm")
    @patch("clica.TOKEN", "test-token-12345")
    def test_import_assets_api_error(
        self, mock_confirm, mock_read_csv, mock_get_folders, mock_post
    ):
    ...
    @patch("click.confirm")
    @patch("clica.TOKEN", "test-token-12345")
    def test_import_assets_type_mapping(
        self, mock_confirm, mock_read_csv, mock_get_folders, mock_post
    ):
...
class TestImportEvidences:
                ...
                with patch("pandas.read_csv", return_value=sample_evidences_csv):
                    with patch("click.confirm", return_value=True):
                        with patch("requests.post") as mock_post:
                            ...
                            runner = CliRunner()
                            result = runner.invoke(
                                import_evidences, ["--file", "test.csv"]
                            )
    ...
    def test_import_evidences_csv_file_not_found(self, mock_token):
        ...
        def test_import_evidences_folders_unavailable(
            self, mock_token, sample_evidences_csv, capsys
        ):
    ...
    def test_import_evidences_partial_success(
        self, mock_token, sample_evidences_csv, capsys
    ):
        """
            ...
            with patch("requests.get") as mock_get:
                with patch("requests.post") as mock_post:
                    ...
                    mock_get_response.json.return_value = {
                        "results": [{"id": 123, "name": evidence_name}]
                    }
                    ...
                    result = runner.invoke(
                        upload_attachment,
                        ["--file", str(test_file), "--name", evidence_name],
                    )
