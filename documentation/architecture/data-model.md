...
## IAM evolution: dynamic groups and is_published per domain

- is_published is replaced by a property of the domain containing the list of object types to publish downward
- we expose role assignments
- we expose builtin groups for all users attached to a domain (generated by create_iam_groups): “EMEA - all users”
- We can create custom role assignments, e.g. between public and "GLOBAL - all users” on role reader
- Users can be attached to any domain (and not only to global)
- For focus mode, we act as a user that is attached to the focused directory. As an option, we can ask to hide objects outside of the domain from "all-users", or objects that are published.
