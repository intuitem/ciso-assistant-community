...
- a `Role` plus a domain-scoped assignment, in the same shape as a human's: either a dedicated role holding exactly the permissions granted, or linked to a shared builtin role so its permissions track that role live.
...
- Scoping federation to SAML as well as OIDC: rejected, SAML has no equivalent convention for presenting a bearer assertion on every API call, so "any IdP" for machine-to-machine auth means any OIDC-compliant one in practice.
