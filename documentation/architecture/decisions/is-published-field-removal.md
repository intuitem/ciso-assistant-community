...
One tenant-immutable builtin role, whose permission list lives in `core/startup.py` and is re-synced at every boot (the list there is the source of truth; the migration-time copy is an initial value only):

- **`BI-RL-BSL` (Baseline reader)** — the organization's shared ground truth: its referentials (frameworks, matrices, threats, reference controls, terminologies, labels, classes, templates, definitions, mappings, libraries), its directory and structure (users, teams, entities, folders), and the standing inventory of the program (assets, applied controls, policies, evidence, vulnerabilities, metric instances, organisation issues and objectives, comments) — but none of its judgments, events, or authored deliverables (assessments, audits, findings, acceptances, exceptions, incidents, documents). The membership test for new models: *is this a fact about the organization, or a judgment it produced?* Facts join the baseline; judgments never do.

It is the default role everywhere a default role is seeded: the fresh-install root, and (in configurable builds) every migrated folder holding published content.

## Edition split

- **Community edition — the default role is hard-coded.** The root folder carries `BI-RL-BSL`, re-pinned by `startup()` at every boot; the folder serializer does not expose the field; no other folder ever carries a default role; and the upgrade migration writes no per-folder posture. This is exact parity with `is_published` for community tenants: folder trees are one level deep there, so the old mechanism could only ever surface root content — which the root's baseline role reproduces. The roles endpoint is read-only in the community build (it serves the default-role display); custom-role management is not a community feature.
- **Enterprise edition — the default role is configurable** (the `CONFIGURABLE_DEFAULT_ROLE` capability, declared by the enterprise settings). The folder form exposes the control on every domain, under the validators above; a fresh install starts with the root row only; the upgrade migration additionally sets `BI-RL-BSL` on every non-enclave folder holding at least one published object of a baseline-list model, so multi-level trees keep their intermediate-domain ambience. The control is a dial: administrators may replace any folder's default role with a narrower view-only role, or clear it — clearing the root's yields a fully explicit-grant posture with no ambient visibility anywhere. Custom-role create/update/delete (with per-folder group provisioning and license-seat enforcement) stays enterprise-only: the enterprise build registers its own roles viewset over the community read-only one.
...
- **The upgrade changes effective access in both directions, and release notes must say so:**
  ...
  - *Narrowings:* models absent from `BI-RL-BSL` (notably security exceptions, incidents, timeline entries, risk acceptances, documents) are no longer ambient anywhere — they are judgments and events, not baseline facts. Their inclusion or exclusion is a deliberate product decision recorded with the role's list, not an accident of migration code.
