...
| Value | Meaning |
|---|---|
| `9999-12-31` | Active / open-ended record (no end date, no deletion date). Used for RT.01.02 deletion date, RT.02.02 end date, and RT.06.01 last assessment date when the underlying timestamp is missing. |

### 3.4 Identifier resolution

When an entity identifier is needed, the export resolves in priority order: **LEI → EUID → VAT → DUNS**. The first non-empty value is used. The identifier type is reported alongside the code in columns that require it.
...
- Values: see [RT.02.02 b_02.02.0140](#b_02020140--storage-of-data) — same table. Full list in [Appendix A — LISTBINARY](#listbinary--yesno).
...
All 16 codification tables consolidated for quick lookup. Python constant names reference `backend/core/dora.py`.
