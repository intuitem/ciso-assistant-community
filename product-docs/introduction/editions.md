...
{% hint style="warning" %}
**Internal users helping fill in an audit _do_ consume a seat.** This includes anyone in your own organisation who is assigned to answer requirements — for example users carrying the built-in **Auditee** role, or any internal teammate split into an audit via the [Assignments / respondent mode](../features/assignments.md) feature. Those users have write access to requirement assessments and evidences inside your domain, and that's the definition of a contributor seat. The third-party exception above applies _only_ to vendor representatives reaching the platform through the auditee surface.
{% endhint %}
