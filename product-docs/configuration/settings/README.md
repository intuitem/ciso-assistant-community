...
- **[Feature flags](feature-flags.md)** — toggles that turn whole product areas on or off. Use these to tailor the UI to what your team actually needs and to keep advanced/experimental capabilities out of sight until you want them.
...
Editing settings requires the `change_globalsettings` permission. In practice, this means a Domain Manager on the global domain or an equivalent custom role on an Enterprise instance.
