...
For framework scoring, the framework `_meta` tab defines the default `min_score`, `max_score`, and `scores_definition`. Individual requirement rows can override those values with their own `min_score`, `max_score`, and `scores_definition` when a framework mixes scoring ranges. Leave those cells blank to inherit the framework-level scale.
