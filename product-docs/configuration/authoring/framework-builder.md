...
- **`compute_result`** — the compliance result this choice maps to. Accepted values: `compliant`, `partially_compliant`, `non_compliant`, `not_applicable`. The same vocabulary is supported by the [Excel import path](framework.md#questions-and-choices) (`compute_result` column in the `questions` sheet, see [`tools/README.md`](https://github.com/intuitem/ciso-assistant-community/blob/main/tools/README.md)), so a questionnaire authored in either place behaves identically. The legacy literals `true` / `1` are still read as `compliant` and `false` / `0` as `non_compliant` for backward compatibility with library YAMLs shipped before the semantic vocabulary existed.
