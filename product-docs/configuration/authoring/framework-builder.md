...
- **`compute_result`** — the compliance result this choice maps to. Accepted values: `compliant`, `partially_compliant`, `non_compliant`, `not_applicable`. The same vocabulary is supported by the [Excel import path](framework.md#questions-and-choices) (`compute_result` column in the `questions` sheet, see [`tools/README.md`](https://github.com/intuitem/ciso-assistant-community/blob/main/tools/README.md)), so a questionnaire authored in either place behaves identically. The legacy literals `true` and `false` are still read as `compliant` and `non_compliant` respectively, for backward compatibility with library YAMLs shipped before the semantic vocabulary existed.

    **Aggregation across choices on a requirement.** When several questions (or several selected choices on a multi-choice question) contribute a `compute_result`, the requirement's result is aggregated _worst-wins_, with `not_applicable` treated as neutral:

    - All contributing choices are `compliant` → **Compliant**.
    - At least one `non_compliant` and at least one `compliant`, or any `partially_compliant` → **Partially compliant**.
    - All non-neutral choices are `non_compliant` → **Non compliant**.
    - Only `not_applicable` contributes → **Not applicable**.
    - No contributing choice → the requirement stays **Not assessed**.

    A choice without a `compute_result` does not contribute to the aggregation, so you can mix "scoring-only" and "result-bearing" choices on the same question.
