...
#### 18. Enable "Trust email without verification claim"

Entra ID does not send the standard `email_verified` claim. Open **Advanced settings** and enable **Trust email without verification claim**, otherwise every login is rejected with "Your identity provider did not confirm your email address". Do this only for an app registration limited to **Accounts in this organizational directory only**, so that the email addresses CISO Assistant trusts are the ones your own tenant administrators control.
