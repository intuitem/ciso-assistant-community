...
If a user has lost access to all their MFA factors (phone wiped, hardware key lost) and has no recovery codes left, an administrator can wipe their MFA from the user's edit page. The link only appears when the target user has MFA enabled and you are not editing yourself. See [Setting up Multi-Factor Authentication (MFA) → Admin recovery](../mfa.md#admin-recovery-resetting-another-users-mfa) for the full procedure.
