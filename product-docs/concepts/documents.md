...
Documents can be **authored in-platform** (a Markdown editor with a draft → published lifecycle) or **uploaded** (an existing PDF/file carried through the same lifecycle). Either way, a document can be linked to the objects it governs and referenced by other documents.
...
A **document container** is the language-independent identity of a document: it holds the document's type, its domain, and its links. Under it sit one **managed document** per language, and each managed document is a chain of **document revisions** — one per saved increment. A new document can be **seeded** from a template, a container can **reference** other containers, and it can be **linked to** the objects it governs. Solid edges are always present; dashed edges are optional.
