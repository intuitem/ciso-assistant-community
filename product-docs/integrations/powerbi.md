...
* **Facts** — the things you count and measure: requirement assessments, applied controls, findings, evidences, incidents, risk scenarios, vulnerabilities, security exceptions, assets, task occurrences, entity assessments, contracts, risk acceptances. When a chart shows a number, it comes from a fact.
* **Dimensions** — what you slice *by*: folders (domains), frameworks, compliance assessments (audits), requirement nodes, risk assessments, perimeters, threats, reference controls, findings assessments, labels, actors (owners and assignees), entities (third parties), solutions, task definitions, campaigns.
...
* **Owners and assignees come through a bridge.** An object can have several owners, so there is no `owner` column on the fact. Load the matching bridge (*Applied Control - Owner*, *Task Node - Assignee*, …) and relate it to the **Actors** dimension, which covers owners that are a user, a team or an entity alike. To count per owner, put a column from the bridge in the visual's values.
...
* **Every table in the template fails with "Unable to connect to the remote server"** — the report is still on the `BaseUrl` it ships with (`https://localhost:8443`), which points at your own machine, so no request ever reaches CISO Assistant. Set it in **Transform data → Manage parameters → `BaseUrl`**, then refresh. The same error appears when the URL is right but the instance is unreachable from that machine — VPN, firewall, or a server that is not running.
* **A table holds fewer rows than CISO Assistant shows** — connector versions up to 1.0.2 imported only the first page of a table when the instance capped API page sizes below 5000 (see [API pagination](../configuration/settings/api-pagination.md)). Install 1.1.0 or later and refresh; the report itself needs no change. Note that row counts can also differ for a legitimate reason: every table is scoped by the permissions of the token's user, so a domain that user cannot see contributes no rows.
* **An ownership bridge has rows that match nothing in Actors** — the Actors table carries entity owners only when **Allow assignments to entities** is enabled in general settings. Objects assigned to an entity while it was enabled keep those links, so their bridge rows join to nothing once it is turned off.
