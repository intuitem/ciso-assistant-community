...
def parse_txt_to_excel(input_file: str, output_file: str):
    ...
    rows.append(
        {"depth": "1", "urn_id": "recital", "name": "Préambule", "description": ""}
    )
        ...
        if re.match(r"^\(*[a-z]\)$", line):
            ...
            if not z_letter.startswith("i") or (
                ...
                if not z_letter.startswith("v") or (
                    ...
                    print(
                        f"ARTICLE : {current_context['article']} :: ANON_D : {current_context['anon_depth']} ; Y_DEPTH : {current_context['y_depth']} ; ARTICLE_D : {current_context['article_depth']} ; CHOOSEN : {depth}"
                    )
    ...
    df = pd.DataFrame(
        rows, columns=["assessable", "depth", "ref_id", "name", "urn_id", "description"]
    )
...
if __name__ == "__main__":
    parse_txt_to_excel(
        "./RTS-DORA-incident-reporting_official_OJ_L_202500301_FR_TXT_extracted.txt",
        "destination_FR.xlsx",
    )
