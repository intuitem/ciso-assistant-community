#!/usr/bin/env python3
"""
Create CIS CSC v8.1 to SCF mapping from STRM data.

This script:
1. Loads CIS CSC v8.1 STRM data
2. Converts ref_ids to lowercase
3. Normalizes relationship values
4. Outputs both CSV and Excel formats

The mapping direction is: CIS CSC control → SCF control
"""
...
def normalize_ref_id(ref: str) -> str:
    """
    Normalize CIS CSC reference ID to lowercase.
...
def create_cis_csc_mapping(input_csv: Path, output_csv: Path, output_xlsx: Path):
    """
    Create CIS CSC v8.1 to SCF mapping.
    """
    print("=" * 70)
    print("CREATING CIS CSC v8.1 TO SCF MAPPING")
    print("=" * 70)
    ...
    print(
        f"Unique source nodes (CIS CSC): {relationship_df['source_node_id'].nunique()}"
    )
...
def main():
    ...
    # Input file
    input_csv = strm_exports / "general-cis-csc-8-1.csv"

    # Output files
    output_csv = script_dir / "cis_csc_scf_mapping.csv"
    output_xlsx = script_dir / "cis_csc_scf_mapping.xlsx"

    # Check input file exists
    if not input_csv.exists():
        print(f"Error: CIS CSC v8.1 STRM file not found: {input_csv}")
        print("Please run extract_strm.py first to generate STRM exports.")
        exit(1)

    # Run the mapping creation
    create_cis_csc_mapping(input_csv, output_csv, output_xlsx)
