#!/usr/bin/env python3
"""
Create PCI-DSS 4.0.1 to SCF mapping from STRM data.

This script:
1. Loads PCI-DSS 4.0.1 STRM data
2. Converts ref_ids to lowercase (e.g., 1.1.1 -> 1.1.1)
3. Normalizes relationship values
4. Outputs both CSV and Excel formats

The mapping direction is: PCI-DSS requirement → SCF control
"""
...
def normalize_ref_id(ref: str) -> str:
    """
    Normalize PCI-DSS reference ID to lowercase.
...
def load_strm_csv(csv_path: Path) -> pd.DataFrame:
    ...
    # Read FDE # as string to preserve values like 1.10
...
def create_pci_dss_mapping(input_csv: Path, output_csv: Path, output_xlsx: Path):
    """
    Create PCI-DSS 4.0.1 to SCF mapping.
    """
    print("=" * 70)
    print("CREATING PCI-DSS 4.0.1 TO SCF MAPPING")
    print("=" * 70)
    ...
    print(
        f"Unique source nodes (PCI-DSS): {relationship_df['source_node_id'].nunique()}"
    )
...
def main():
    ...
    # Input file
    input_csv = strm_exports / "general-pci-dss-4-0-1.csv"

    # Output files
    output_csv = script_dir / "pci_dss_scf_mapping.csv"
    output_xlsx = script_dir / "pci_dss_scf_mapping.xlsx"

    # Check input file exists
    if not input_csv.exists():
        print(f"Error: PCI-DSS 4.0.1 STRM file not found: {input_csv}")
        print("Please run extract_strm.py first to generate STRM exports.")
        exit(1)

    # Run the mapping creation
    create_pci_dss_mapping(input_csv, output_csv, output_xlsx)
