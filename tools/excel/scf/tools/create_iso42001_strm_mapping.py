#!/usr/bin/env python3
"""
Create ISO 42001:2023 to SCF mapping from STRM data.

This script:
1. Loads ISO 42001:2023 STRM data
2. Converts ref_ids to lowercase
3. Normalizes relationship values
4. Outputs both CSV and Excel formats

The mapping direction is: ISO 42001 clause → SCF control
"""
...
def normalize_ref_id(ref: str) -> str:
    """
    Normalize ISO 42001 reference ID to lowercase and strip parenthetical suffixes.
...
def load_strm_csv(csv_path: Path) -> pd.DataFrame:
    ...
    # Read FDE # as string to preserve values like 4.10
...
def create_iso42001_mapping(input_csv: Path, output_csv: Path, output_xlsx: Path):
    """
    Create ISO 42001:2023 to SCF mapping.
    """
    print("=" * 70)
    print("CREATING ISO 42001:2023 TO SCF MAPPING")
    print("=" * 70)
    ...
    print(
        f"Unique source nodes (ISO 42001): {relationship_df['source_node_id'].nunique()}"
    )
...
def main():
    ...
    # Input file
    input_csv = strm_exports / "general-iso-42001-2023.csv"

    # Output files
    output_csv = script_dir / "iso42001_scf_mapping.csv"
    output_xlsx = script_dir / "iso42001_scf_mapping.xlsx"

    # Check input file exists
    if not input_csv.exists():
        print(f"Error: ISO 42001:2023 STRM file not found: {input_csv}")
        print("Please run extract_strm.py first to generate STRM exports.")
        exit(1)

    # Run the mapping creation
    create_iso42001_mapping(input_csv, output_csv, output_xlsx)
