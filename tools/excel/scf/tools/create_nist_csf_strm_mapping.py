#!/usr/bin/env python3
"""
Create NIST CSF 2.0 to SCF mapping from STRM data.

This script:
1. Loads NIST CSF 2.0 STRM data
2. Converts ref_ids to lowercase (e.g., GV.OC-01 -> gv.oc-01)
3. Normalizes relationship values
4. Outputs both CSV and Excel formats

The mapping direction is: NIST CSF 2.0 control → SCF control
"""
...
def normalize_ref_id(ref: str) -> str:
    """
    Normalize NIST CSF 2.0 reference ID to lowercase.
...
def create_nist_csf_mapping(input_csv: Path, output_csv: Path, output_xlsx: Path):
    """
    Create NIST CSF 2.0 to SCF mapping.
    """
    print("=" * 70)
    print("CREATING NIST CSF 2.0 TO SCF MAPPING")
    print("=" * 70)

    # Load NIST CSF 2.0 data
    ...
    print(
        f"Unique source nodes (NIST CSF): {relationship_df['source_node_id'].nunique()}"
    )
...
def main():
    ...
    # Input file
    input_csv = strm_exports / "general-nist-csf-2-0.csv"

    # Output files
    output_csv = script_dir / "nist_csf_scf_mapping.csv"
    output_xlsx = script_dir / "nist_csf_scf_mapping.xlsx"

    # Check input file exists
    if not input_csv.exists():
        print(f"Error: NIST CSF 2.0 STRM file not found: {input_csv}")
        print("Please run extract_strm.py first to generate STRM exports.")
        exit(1)

    # Run the mapping creation
    create_nist_csf_mapping(input_csv, output_csv, output_xlsx)
