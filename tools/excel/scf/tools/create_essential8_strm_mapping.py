#!/usr/bin/env python3
"""
Create Australia Essential 8 to SCF mapping from STRM data.

This script:
1. Loads Australia Essential 8 STRM data
2. Converts ref_ids to lowercase (e.g., ML1-P1 -> ml1-p1)
3. Normalizes relationship values
4. Outputs both CSV and Excel formats

The mapping direction is: Essential 8 control → SCF control
"""
...
def normalize_ref_id(ref: str) -> str:
    """
    Normalize Essential 8 reference ID to lowercase.
...
def create_essential8_mapping(input_csv: Path, output_csv: Path, output_xlsx: Path):
    """
    Create Australia Essential 8 to SCF mapping.
    """
    print("=" * 70)
    print("CREATING AUSTRALIA ESSENTIAL 8 TO SCF MAPPING")
    print("=" * 70)

    # Load Essential 8 data
    ...
    print(
        f"Unique source nodes (Essential 8): {relationship_df['source_node_id'].nunique()}"
    )
...
def main():
    ...
    # Input file
    input_csv = strm_exports / "apac-australia-essential-8.csv"

    # Output files
    output_csv = script_dir / "essential8_scf_mapping.csv"
    output_xlsx = script_dir / "essential8_scf_mapping.xlsx"

    # Check input file exists
    if not input_csv.exists():
        print(f"Error: Australia Essential 8 STRM file not found: {input_csv}")
        print("Please run extract_strm.py first to generate STRM exports.")
        exit(1)

    # Run the mapping creation
    create_essential8_mapping(input_csv, output_csv, output_xlsx)
