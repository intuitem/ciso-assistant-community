#!/usr/bin/env python3
"""
Create SOC2 (AICPA TSC 2017) to SCF mapping from STRM data.

This script:
1. Loads AICPA TSC 2017 STRM data
2. Converts POF (Point of Focus) suffixes to dotted notation (e.g., CC1.1-POF1 -> cc1.1.1)
3. Normalizes relationship values
4. Outputs both CSV and Excel formats

The mapping direction is: SOC2 criteria → SCF control
"""
...
def convert_pof_to_dotted(ref: str) -> str:
    """
    Convert POF (Point of Focus) suffix to dotted notation and lowercase.

    Examples:
        CC1.1-POF1 -> cc1.1.1
        CC1.1-POF3 -> cc1.1.3
        A1.2-POF10 -> a1.2.10
        CC6.1 -> cc6.1 (no change, just lowercase)
    """
    if not ref:
        return ref
    ...
    # Find '-POF' and convert to dotted notation
    pof_idx = ref.find("-POF")
    if pof_idx > 0:
        base = ref[:pof_idx]
        pof_num = ref[pof_idx + 4 :]  # Get the number after '-POF'
        ref = f"{base}.{pof_num}"
...
def load_strm_csv(csv_path: Path) -> pd.DataFrame:
    ...
    # Convert POF suffixes to dotted notation (e.g., CC1.1-POF1 -> cc1.1.1)
    df["FDE #"] = df["FDE #"].apply(convert_pof_to_dotted)
...
def create_soc2_mapping(input_csv: Path, output_csv: Path, output_xlsx: Path):
    """
    Create SOC2 to SCF mapping.
    """
    print("=" * 70)
    print("CREATING SOC2 (AICPA TSC 2017) TO SCF MAPPING")
    print("=" * 70)
    ...
    print(f"Unique source nodes (SOC2): {relationship_df['source_node_id'].nunique()}")
...
def main():
    ...
    # Input file
    input_csv = strm_exports / "general-aicpa-tsc-2017.csv"

    # Output files
    output_csv = script_dir / "soc2_scf_mapping.csv"
    output_xlsx = script_dir / "soc2_scf_mapping.xlsx"

    # Check input file exists
    if not input_csv.exists():
        print(f"Error: AICPA TSC STRM file not found: {input_csv}")
        print("Please run extract_strm.py first to generate STRM exports.")
        exit(1)

    # Run the mapping creation
    create_soc2_mapping(input_csv, output_csv, output_xlsx)
