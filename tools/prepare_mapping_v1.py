...
Usage:
    python prepare_mapping.py source.yaml target.yaml [--compat]

Arguments:
    source.yaml       The source YAML file
    target.yaml       The target YAML file

Optional:
    --compat          Enables compatibility mode for legacy naming (mapping from ... to ...)
"""
...
def generate_mapping_excel(source_yaml, target_yaml, compat=False):
    packager = "intuitem"
    ...
    if compat:
        ...
        ref_id = "mapping-" + source_ref_id + "-and-" + target_ref_id
        name = f"{source_framework_ref_id} <-> {target_framework_ref_id}"
...
def main():
    ...
    parser.add_argument("--compat", action="store_true", help="Use compatibility naming format (source-to-target)",
    )
    args = parser.parse_args()
    
    if args.compat:
        print("🟢 Compat Mode : ON (Legacy naming [mapping from ... to ...])")
    else:
        print("🔴 Compat Mode : OFF (New naming [mapping between ... and ...])")

    generate_mapping_excel(args.source_yaml, args.target_yaml, compat=args.compat)
