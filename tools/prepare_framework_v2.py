...
def validate_urn_root(urn_root):
    if not re.fullmatch(r"[a-z0-9._-]+", urn_root):
        raise ValueError(
            f"(validate_urn_root) Invalid URN root \"{urn_root}\" : Only lowercase alphanumeric characters, '-', '_', and '.' are allowed."
        )
...
def validate_ref_id(ref_id):
    if not re.fullmatch(r"[a-zA-Z0-9._-]+", ref_id):
        raise ValueError(
            f"Invalid Ref. ID \"{ref_id}\" : Only alphanumeric characters, '-', '_', and '.' are allowed."
        )
...
def validate_implementation_groups(
    impl_groups, context="implementation_groups", is_in_excel_config=False
):
    if not isinstance(impl_groups, list) or not impl_groups:
        if is_in_excel_config:
            raise ValueError(
                f'(validate_implementation_groups) Table in sheet "{context}" musn\'t be empty.\n'
                '💡 Tip: Fill this sheet or, if you don\'t use it, disable it by adding "#" in front of the sheet name.'
            )
        else:
            raise ValueError(
                f'(validate_implementation_groups) Field "{context}" must be a non-empty list if defined.'
            )
    ...
    for i, group in enumerate(impl_groups, start=1):
        if "ref_id" not in group or not str(group["ref_id"]).strip():
            if is_in_excel_config:
                raise ValueError(
                    f'(validate_implementation_groups) Missing or empty "ref_id" in table of sheet "{context}" (Row #{i + 2})'
                )
            else:
                raise ValueError(
                    f'(validate_implementation_groups) Missing or empty "ref_id" in {context} #{i}'
                )

        if group["ref_id"] in impl_groups_ref_ids:
            if is_in_excel_config:
                raise ValueError(
                    f'(validate_implementation_groups) ref_id "{group["ref_id"]}" is duplicated in table of sheet "{context}" (Row #{i + 2})'
                )
            else:
                raise ValueError(
                    f'(validate_implementation_groups) ref_id "{group["ref_id"]}" is duplicated in {context} (#{i})'
                )
        ...
        try:
            ...
            if is_in_excel_config:
                raise ValueError(
                    f'(validate_implementation_groups) {e} in table of sheet "{context}" (Row #{i + 2})'
                )
            else:
                raise ValueError(
                    f"(validate_implementation_groups) {e} in {context} #{i}"
                )

        if "name" not in group or not str(group["name"]).strip():
            if is_in_excel_config:
                raise ValueError(
                    f'(validate_implementation_groups) Missing or empty "name" in table of sheet "{context}" (Row #{i + 2})'
                )
            else:
                raise ValueError(
                    f'(validate_implementation_groups) Missing or empty "name" in {context} #{i}'
                )

        if "description" not in group or not str(group["description"]).strip():
            if is_in_excel_config:
                print(
                    f'⚠️  [WARNING] (validate_implementation_groups) Missing or empty "description" in table of sheet "{context}" (Row #{i + 2})'
                )
            else:
                print(
                    f'⚠️  [WARNING] (validate_implementation_groups) Missing or empty "description" in {context} #{i}'
                )
...
def validate_yaml_extra_locales(data):
    ...
    if not isinstance(extra_locales, list) or not extra_locales:
        raise ValueError(
            '(validate_yaml_extra_locales) Field "extra_locales" must be a non-empty list if defined.'
        )
    ...
    for i, locale_entry in enumerate(extra_locales, start=1):
        if not isinstance(locale_entry, dict) or len(locale_entry) != 1:
            raise ValueError(
                f'(validate_yaml_extra_locales) Each entry in "extra_locales" must be a dict with exactly one locale code key (entry #{i})'
            )

        if framework_locale in locale_entry.keys():
            print(
                f'⚠️  [WARNING] (validate_yaml_extra_locales) Locale "{framework_locale}" is already the framework\'s main locale.'
        ...
        for loc in extra_locales_list:
            if loc in locale_entry.keys():
                raise ValueError(
                    f'(validate_yaml_extra_locales) Locale "{loc}" is duplicated (#{i})'
                )
        ...
        for loc_code, loc_data in locale_entry.items():
            if not is_valid_locale(loc_code):
                raise ValueError(
                    f'(validate_yaml_extra_locales) Invalid locale code "{loc_code}" in extra_locales entry #{i}'
                )

            if not isinstance(loc_data, dict) or not loc_data:
                raise ValueError(
                    f'(validate_yaml_extra_locales) Locale data for "{loc_code}" must be a non-empty dict (entry #{i})'
                )
            ...
            for req_field in ["framework_name", "description", "copyright"]:
                if req_field not in loc_data or str(loc_data[req_field]).strip() == "":
                    print(
                        f'⚠️  [WARNING] (validate_yaml_extra_locales) Missing or empty field "{req_field}" in extra_locales for locale "{loc_code}" (entry #{i})'
                    )
            ...
            if loc_impl_groups is not None:
                if not impl_groups_main:
                    print(
                        f'⚠️  [WARNING] (validate_yaml_extra_locales) "implementation_groups" defined in locale "{loc_code}" but missing in framework.'
                        '\n\t     ⏩ Skipping locale\'s "implementation_groups"...'
                    )
                    continue
                if not impl_groups_main_sheet:
                    print(
                        f'⚠️  [WARNING] (validate_yaml_extra_locales) "implementation_groups" defined in locale "{loc_code}" but missing "implementation_groups_sheet_base_name" field in framework.'
                        '\n\t     ⏩ Skipping locale\'s "implementation_groups"...'
                    )
                    continue

                validate_implementation_groups(
                    loc_impl_groups, f'implementation_groups in locale "{loc_code}"'
                )
                ...
                for group in loc_impl_groups:
                    if all(group["ref_id"] != fg["ref_id"] for fg in impl_groups_main):
                        raise ValueError(
                            f'ref_id "{group["ref_id"]}" in locale "{loc_code}" does not exist in framework implementation_groups.'
                        )
...
def validate_yaml_data(data):
    ...
    if not is_valid_locale(data.get("locale")):
        raise ValueError(
            f'(validate_yaml_data) Invalid locale code "{data.get("locale")}" in "locale" entry'
        )
    ...
    if impl_base:
        if impl_groups is not None:
            ...
            print(
                f'ℹ️  [INFO] Implementation groups sheets will be added using base name: "{impl_base}"'
            )
        else:
            print(
                f'⚠️  [WARNING] (validate_yaml_data) "implementation_groups_sheet_base_name" defined but missing "implementation_groups" field in YAML file.'
    ...
    # If there's "implementation_groups" but no "implementation_groups_sheet_base_name", raise a warning
        ...
        print(
            f'⚠️  [WARNING] (validate_yaml_data) "implementation_groups" defined but missing "implementation_groups_sheet_base_name" field in YAML file.'
...
def validate_excel_data(wb):
    if "base" not in wb.sheetnames or not is_sheet_enabled("base"):
        raise ValueError(
            '(validate_excel_data) Missing or commented-out required sheet: "base"'
        )
    ...
    if impl_base:
        if impl_base not in wb.sheetnames:
            if f"#{impl_base}" in wb.sheetnames:
                print(
                    f'⚠️  [WARNING] (validate_excel_data) "implementation_groups_sheet_base_name" defined but the "{impl_base}" sheet is disabled in Excel file.'
                    "\n\t     A blank implementation groups sheet will be created in the output Excel file."
                )
            else:
                print(
                    f'⚠️  [WARNING] (validate_excel_data) "implementation_groups_sheet_base_name" defined but missing "{impl_base}" sheet in Excel file.'
                    "\n\t     A blank implementation groups sheet will be created in the output Excel file."
                )
        else:
            ...
            validate_implementation_groups(
                impl_groups_main, impl_base, is_in_excel_config=True
            )
    elif "imp_grp" in wb.sheetnames:
        print(
            f'⚠️  [WARNING] (validate_excel_data) "imp_grp" sheet enabled but missing "implementation_groups_sheet_base_name" field in Excel file.'
            '\n\t     ⏩ Skipping sheet "imp_grp"...'
        )
        print(
            '💡 Tip: Remember to remove the "#" in front of "implementation_groups_sheet_base_name" in the "base" sheet if you want to use the implementation groups sheet.'
        )
    ...
    for sheet in wb.sheetnames:
        ...
        if sheet.startswith("base_"):
            ...
            if not extract_locale_suffix(sheet):
                raise ValueError(
                    f"(validate_excel_data) Invalid locale code in sheet name '{sheet}' (parsed as '{loc}')"
                )

            # Check duplicate locale
            if loc in base_extra_locales_list:
                raise ValueError(
                    f'(validate_excel_data) Locale "{loc}" is duplicated (Sheet: "{sheet})"'
                )
            ...
            if locale_main == loc:
                print(
                    f'⚠️  [WARNING] (validate_excel_data) Locale "{loc}" is already the framework\'s main locale.'
                    f'\n\t     ⏩ Skipping "{sheet}"...'
                )
                continue
            ...
            for field in ["framework_name", "description", "copyright"]:
                if not loc_data.get(field, "") or loc_data.get(field, "").strip() == "":
                    print(
                        f'⚠️  [WARNING] (validate_excel_data) Missing or empty "{field}" in locale "{loc}" (Sheet: "{sheet}")'
                    )

        elif sheet.startswith("imp_grp_") and impl_base:
            if not impl_base:
                print(
                    f'⚠️  [WARNING] (validate_excel_data) "{sheet}" defined but missing "implementation_groups_sheet_base_name" field.'
                    f'\n\t     ⏩ Skipping "{sheet}"...'
                )
                continue
            ...
            if not impl_groups_main:
                print(
                    f'⚠️  [WARNING] (validate_excel_data) "{sheet}" defined but "imp_grp" sheet is missing.\n'
                    f'\t     ⏩ Skipping "{sheet}"...'
                )
                continue
            ...
            validate_implementation_groups(
                impl_groups_loc, sheet, is_in_excel_config=True
            )
...
def format_excel_data_as_config(wb):
    ...
    for sheet in wb.sheetnames:
        ...
        if sheet.startswith("base_"):
            ...
            if localized_imp_groups:
                extra_locales_dict[locale]["implementation_groups"] = (
                    localized_imp_groups
                )

    # Reformat extra_locales as list of {locale: data}
    extra_locales = (
        [{loc: data} for loc, data in extra_locales_dict.items()]
        if extra_locales_dict
        else None
    )
    ...
    config = {
        ...
        "implementation_groups_sheet_base_name": base_data.get(
            "implementation_groups_sheet_base_name"
        ),
...
def create_excel_from_yaml(yaml_path, output_excel=None):
    ...
    if output_excel is None:
        ...
        yaml_output_name
        and str(yaml_output_name).strip()
        and output_excel != yaml_output_name.strip()
    ):
        print(
            f'ℹ️  [INFO] Overriding YAML "excel_file_name" with command line argument.'
        )
...
def create_excel_from_excel(excel_path, output_excel=None):
    ...
    if output_excel is None:
        ...
        excel_output_name
        and str(excel_output_name).strip()
        and output_excel != excel_output_name.strip()
    ):
        print(
            f'ℹ️  [INFO] Overriding Excel "excel_file_name" with command line argument.'
        )
...
def create_excel(data, output_excel=None):
    ...
    if extra_locales:
        for locale_entry in extra_locales:
            if (
                locale in locale_entry.keys()
            ):  # Ignore if the extra locale is the same as the framework's locale
                continue
            for loc_code, loc_data in locale_entry.items():
                # Only add fields that are present and non-empty
                if (
                    "framework_name" in loc_data
                    and str(loc_data["framework_name"]).strip()
                ):
    ...
    if impl_group_base:
        framework_meta_sheet.append(
            ["implementation_groups_definition", impl_group_base]
        )
        ...
        for locale_entry in extra_locales:
            ...
            for loc_code, loc_data in locale_entry.items():
                if (
                    ...
                    framework_meta_sheet.append(
                        [f"name[{loc_code}]", loc_data["framework_name"]]
                    )
                if "description" in loc_data and str(loc_data["description"]).strip():
                    framework_meta_sheet.append(
                        [f"description[{loc_code}]", loc_data["description"]]
                    )
            ...
            for rid, langs_data in lang_impl_groups_map.items():
                if rid in ref_id_to_row:
                    ...
                    for lang_code, texts in langs_data.items():
                        ...
                        if name_col:
                            impl_content_sheet.cell(
                                row=row_num, column=name_col, value=texts["name"]
                            )
                        if desc_col:
                            impl_content_sheet.cell(
                                row=row_num, column=desc_col, value=texts["description"]
                            )
    ...
    try:
        ...
        print(
            "💡 Tip: Make sure the Excel file is not open in another program and that you have write permission."
        )
        sys.exit(1)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate Excel file from YAML configuration"
    )
    parser.add_argument(
        "-i",
        "--input",
        default="prepare_framework_v2_config.xlsx",
        help="Input YAML or Excel configuration file (default: prepare_framework_v2_config.xlsx)",
    )
    parser.add_argument(
        "-o", "--output", default=None, help="Output Excel file name (optional)"
    )
    ...
    try:
        ...
        if input_path.lower().endswith((".yaml", ".yml")):
            print(
                f'ℹ️  [INFO] YAML configuration file "{os.path.basename(input_path)}" will be used'
            )
            create_excel_from_yaml(input_path, args.output)
        elif input_path.lower().endswith(".xlsx"):
            print(
                f'ℹ️  [INFO] Excel configuration file "{os.path.basename(input_path)}" will be used'
            )
            create_excel_from_excel(input_path, args.output)
        else:
            raise ValueError(
                "Unsupported input file format. Must be .yaml, .yml, or .xlsx"
            )
