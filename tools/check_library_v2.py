...
from types import (
    MappingProxyType,
)  # Prefer over "frozendict" to avoid importing an external library
...
class ConsoleContext:  # Maybe rename it to "Logger" and create a "Context/Config" object to carry compatibility & conversion settings, etc. in one object
    ...
    def get_warning_count_by_sheet(self) -> Dict[str, int]:
        ...
        return {
            fn: len(self.warning_messages.get(fn, [])) for fn in self._all_sheet_names
        }

    def get_verbose_count_by_sheet(self) -> Dict[str, int]:
        """Return dict {sheet_name: verbose count}, including 0 where applicable."""
        return {
            fn: len(self.verbose_messages.get(fn, [])) for fn in self._all_sheet_names
        }
...
# [META] TTP Catalog supported keys
class TTPCatalogMetaKeys(MetaKey):
    BASE_URN = "base_urn"
    GROUPING_DEFINITION = "grouping_definition"


# [META] TTP Groups supported keys
class TTPGroupsMetaKeys(MetaKey):
    NAME = "name"


# [META] Tactics supported keys
class TacticsMetaKeys(MetaKey):
    BASE_URN = "base_urn"
    CATALOG_URN = "catalog_urn"


# [META] Techniques supported keys
class TechniquesMetaKeys(MetaKey):
    BASE_URN = "base_urn"
    CATALOG_URN = "catalog_urn"
    TACTICS_BASE_URN = "tactics_base_urn"
    ...
    def __post_init__(self):
        ...
        for keys in key_groups:
            if not all(isinstance(key, self.key_enum) for key in keys):
                raise TypeError(
                    f"All schema keys must come from {self.key_enum.__name__}"
                )
...
META_SHEET_SCHEMAS: Mapping[MetaTypes, MetaSheetSchema] = MappingProxyType(
    {
        MetaTypes.LIBRARY: MetaSheetSchema(
            ...
            expected_keys=(
                LibraryMetaKeys.URN,
                LibraryMetaKeys.VERSION,
                LibraryMetaKeys.LOCALE,
                LibraryMetaKeys.REF_ID,
                LibraryMetaKeys.NAME,
                LibraryMetaKeys.DESCRIPTION,
                LibraryMetaKeys.COPYRIGHT,
                LibraryMetaKeys.PROVIDER,
                LibraryMetaKeys.PACKAGER,
            ),
        ...
        MetaTypes.FRAMEWORK: MetaSheetSchema(
            ...
            expected_keys=(
                FrameworkMetaKeys.URN,
                FrameworkMetaKeys.REF_ID,
                FrameworkMetaKeys.NAME,
                FrameworkMetaKeys.DESCRIPTION,
                FrameworkMetaKeys.BASE_URN,
            ),
            optional_keys=(
                FrameworkMetaKeys.MIN_SCORE,
                FrameworkMetaKeys.MAX_SCORE,
                FrameworkMetaKeys.SCORES_DEFINITION,
                FrameworkMetaKeys.IMPLEMENTATION_GROUPS_DEFINITION,
                FrameworkMetaKeys.ANSWERS_DEFINITION,
            ),
        ...
        MetaTypes.TTP_CATALOG: MetaSheetSchema(
            key_enum=TTPCatalogMetaKeys,
            expected_keys=(TTPCatalogMetaKeys.BASE_URN,),
            optional_keys=(TTPCatalogMetaKeys.GROUPING_DEFINITION,),
        ),
        MetaTypes.TTP_GROUPS: MetaSheetSchema(
            key_enum=TTPGroupsMetaKeys,
            expected_keys=(TTPGroupsMetaKeys.NAME,),
        ),
        MetaTypes.TACTICS: MetaSheetSchema(
            key_enum=TacticsMetaKeys,
            expected_keys=(TacticsMetaKeys.BASE_URN, TacticsMetaKeys.CATALOG_URN),
        ),
        MetaTypes.TECHNIQUES: MetaSheetSchema(
            key_enum=TechniquesMetaKeys,
            expected_keys=(TechniquesMetaKeys.BASE_URN,),
            optional_keys=(
                TechniquesMetaKeys.CATALOG_URN,
                TechniquesMetaKeys.TACTICS_BASE_URN,
            ),
        ),
        ...
        MetaTypes.RISK_MATRIX: MetaSheetSchema(
            ...
            expected_keys=(
                RiskMatrixMetaKeys.URN,
                RiskMatrixMetaKeys.REF_ID,
                RiskMatrixMetaKeys.NAME,
        ...
        MetaTypes.REQUIREMENT_MAPPING_SET: MetaSheetSchema(
            ...
            expected_keys=(
                RequirementMappingSetMetaKeys.URN,
                RequirementMappingSetMetaKeys.REF_ID,
                RequirementMappingSetMetaKeys.NAME,
                RequirementMappingSetMetaKeys.DESCRIPTION,
                RequirementMappingSetMetaKeys.SOURCE_FRAMEWORK_URN,
                RequirementMappingSetMetaKeys.SOURCE_NODE_BASE_URN,
                RequirementMappingSetMetaKeys.TARGET_FRAMEWORK_URN,
                RequirementMappingSetMetaKeys.TARGET_NODE_BASE_URN,
            ),
        ),
...
# [CONTENT] TTP Catalog supported columns
class TTPCatalogContentColumns(ContentColumn):
    REF_ID = "ref_id"
    NAME = "name"
    DESCRIPTION = "description"
    ANNOTATION = "annotation"
...
# [CONTENT] TTP Groups supported columns
class TTPGroupsContentColumns(ContentColumn):
    REF_ID = "ref_id"
    NAME = "name"
    DESCRIPTION = "description"
    DIMENSION = "dimension"
...
# [CONTENT] Tactics supported columns
class TacticsContentColumns(ContentColumn):
    REF_ID = "ref_id"
    NAME = "name"
    DESCRIPTION = "description"
    ANNOTATION = "annotation"
...
# [CONTENT] Techniques supported columns
class TechniquesContentColumns(ContentColumn):
    REF_ID = "ref_id"
    NODE_ID = "node_id"
    NAME = "name"
    DESCRIPTION = "description"
...
FRAMEWORK_CONTENT_COLUMN_CONSTRAINTS: Mapping[
    ContentColumn, ContentColumnConstraints
] = MappingProxyType(
    {
        FrameworkContentColumns.DEPTH: ContentColumnConstraints(
            integer_only=True, min_value=1
        ),
        ...
        FrameworkContentColumns.ASSESSABLE: ContentColumnConstraints(
            allowed_values=("x", "X")
        ),
        FrameworkContentColumns.IMPORTANCE: ContentColumnConstraints(
            allowed_values=("mandatory", "recommended", "nice_to_have"),
        ),
        FrameworkContentColumns.WEIGHT: ContentColumnConstraints(
            integer_only=True, min_value=1
        ),
        FrameworkContentColumns.MIN_SCORE: ContentColumnConstraints(
            integer_only=True, min_value=0
        ),
        FrameworkContentColumns.MAX_SCORE: ContentColumnConstraints(
            integer_only=True, min_value=0
        ),
...
THREATS_CONTENT_COLUMN_CONSTRAINTS: Mapping[ContentColumn, ContentColumnConstraints] = (
    MappingProxyType(
        {
...
# [CONTENT] TTP Catalog column constraints
TTP_CATALOG_CONTENT_COLUMN_CONSTRAINTS: Mapping[
    ContentColumn, ContentColumnConstraints
] = MappingProxyType(
    {
        TTPCatalogContentColumns.REF_ID: ContentColumnConstraints(unique=True),
    }
)


# [CONTENT] TTP Groups column constraints
TTP_GROUPS_CONTENT_COLUMN_CONSTRAINTS: Mapping[
    ContentColumn, ContentColumnConstraints
] = MappingProxyType(
    {
        TTPGroupsContentColumns.REF_ID: ContentColumnConstraints(unique=True),
    }
)


# [CONTENT] Tactics column constraints
TACTICS_CONTENT_COLUMN_CONSTRAINTS: Mapping[ContentColumn, ContentColumnConstraints] = (
    MappingProxyType(
        {
            TacticsContentColumns.REF_ID: ContentColumnConstraints(unique=True),
        }
    )
)


# [CONTENT] Techniques column constraints
TECHNIQUES_CONTENT_COLUMN_CONSTRAINTS: Mapping[
    ContentColumn, ContentColumnConstraints
] = MappingProxyType(
    {
        TechniquesContentColumns.REF_ID: ContentColumnConstraints(unique=True),
    }
)


# [CONTENT] Reference Controls column constraints
REFERENCE_CONTROLS_CONTENT_COLUMN_CONSTRAINTS: Mapping[
    ContentColumn, ContentColumnConstraints
] = MappingProxyType(
    {
...
RISK_MATRIX_CONTENT_COLUMN_CONSTRAINTS: Mapping[
    ContentColumn, ContentColumnConstraints
] = MappingProxyType(
    {
...
IMPLEMENTATION_GROUPS_CONTENT_COLUMN_CONSTRAINTS: Mapping[
    ContentColumn, ContentColumnConstraints
] = MappingProxyType(
    {
        ImplementationGroupsContentColumns.REF_ID: ContentColumnConstraints(
            unique=True
        ),
        ImplementationGroupsContentColumns.DEFAULT_SELECTED: ContentColumnConstraints(
            allowed_values=("x", "X")
        ),
    }
)
...
REQUIREMENT_MAPPING_SET_CONTENT_COLUMN_CONSTRAINTS: Mapping[
    ContentColumn, ContentColumnConstraints
] = MappingProxyType(
    {
...
SCORES_CONTENT_COLUMN_CONSTRAINTS: Mapping[ContentColumn, ContentColumnConstraints] = (
    MappingProxyType(
        {
            ScoresContentColumns.SCORE: ContentColumnConstraints(
                integer_only=True, min_value=0, unique=True
            ),
        }
    )
)
...
ANSWERS_CONTENT_COLUMN_CONSTRAINTS: Mapping[ContentColumn, ContentColumnConstraints] = (
    MappingProxyType(
        {
            ...
            AnswersContentColumns.ADD_SCORE: ContentColumnConstraints(
                integer_only=True, split_regex=CommonSeparatorRegex.LF, min_value=0
            ),
...
URN_PREFIX_CONTENT_COLUMN_CONSTRAINTS: Mapping[
    ContentColumn, ContentColumnConstraints
] = MappingProxyType(
    {
    ...
    def __post_init__(self):
        ...
        for columns in column_groups:
            if not all(isinstance(column, self.column_enum) for column in columns):
                raise TypeError(
                    f"All schema columns must come from {self.column_enum.__name__}"
                )

        if not all(
            isinstance(column, self.column_enum) for column in self.column_constraints
        ):
            raise TypeError(
                f"All constraint columns must come from {self.column_enum.__name__}"
            )
...
CONTENT_SHEET_SCHEMAS: Mapping[MetaTypes, ContentSheetSchema] = MappingProxyType(
    {
        MetaTypes.FRAMEWORK: ContentSheetSchema(
            ...
            required_columns=(
                FrameworkContentColumns.DEPTH,
            ),  # "assessable" isn't there because it has a special behavior (column is mandatory, but not the value in column)
            column_constraints=FRAMEWORK_CONTENT_COLUMN_CONSTRAINTS,
            optional_columns=(
                FrameworkContentColumns.REF_ID,
                FrameworkContentColumns.URN_ID,
                FrameworkContentColumns.NAME,
                FrameworkContentColumns.DESCRIPTION,
                FrameworkContentColumns.ANNOTATION,
                FrameworkContentColumns.TYPICAL_EVIDENCE,
                FrameworkContentColumns.IMPORTANCE,
                FrameworkContentColumns.WEIGHT,
                FrameworkContentColumns.MIN_SCORE,
                FrameworkContentColumns.MAX_SCORE,
                FrameworkContentColumns.SCORES_DEFINITION,
                FrameworkContentColumns.IMPLEMENTATION_GROUPS,
                FrameworkContentColumns.QUESTIONS,
                FrameworkContentColumns.ANSWER,
                FrameworkContentColumns.DEPENDS_ON,
                FrameworkContentColumns.CONDITION,
                FrameworkContentColumns.THREATS,
                FrameworkContentColumns.REFERENCE_CONTROLS,
            ),
        ...
        MetaTypes.THREATS: ContentSheetSchema(
            ...
            optional_columns=(
                ThreatsContentColumns.DESCRIPTION,
                ThreatsContentColumns.ANNOTATION,
            ),
            translatable_columns=(
                ThreatsContentColumns.NAME,
                ThreatsContentColumns.DESCRIPTION,
                ThreatsContentColumns.ANNOTATION,
            ),
        ),
        MetaTypes.TTP_CATALOG: ContentSheetSchema(
            column_enum=TTPCatalogContentColumns,
            required_columns=(
                TTPCatalogContentColumns.REF_ID,
                TTPCatalogContentColumns.NAME,
            ),
            column_constraints=TTP_CATALOG_CONTENT_COLUMN_CONSTRAINTS,
            optional_columns=(
                TTPCatalogContentColumns.DESCRIPTION,
                TTPCatalogContentColumns.ANNOTATION,
            ),
            translatable_columns=(
                TTPCatalogContentColumns.NAME,
                TTPCatalogContentColumns.DESCRIPTION,
        ...
        MetaTypes.TTP_GROUPS: ContentSheetSchema(
            column_enum=TTPGroupsContentColumns,
            required_columns=(
                TTPGroupsContentColumns.REF_ID,
                TTPGroupsContentColumns.NAME,
            ),
            column_constraints=TTP_GROUPS_CONTENT_COLUMN_CONSTRAINTS,
            optional_columns=(
                TTPGroupsContentColumns.DESCRIPTION,
                TTPGroupsContentColumns.DIMENSION,
            ),
            translatable_columns=(
                TTPGroupsContentColumns.NAME,
                TTPGroupsContentColumns.DESCRIPTION,
        ...
        MetaTypes.TACTICS: ContentSheetSchema(
            column_enum=TacticsContentColumns,
            required_columns=(TacticsContentColumns.REF_ID, TacticsContentColumns.NAME),
            column_constraints=TACTICS_CONTENT_COLUMN_CONSTRAINTS,
            optional_columns=(
                TacticsContentColumns.DESCRIPTION,
                TacticsContentColumns.ANNOTATION,
            ),
            translatable_columns=(
                TacticsContentColumns.NAME,
                TacticsContentColumns.DESCRIPTION,
                TacticsContentColumns.ANNOTATION,
            ),
        ),
        MetaTypes.TECHNIQUES: ContentSheetSchema(
            column_enum=TechniquesContentColumns,
            required_columns=(
                TechniquesContentColumns.REF_ID,
                TechniquesContentColumns.NAME,
            ),
            column_constraints=TECHNIQUES_CONTENT_COLUMN_CONSTRAINTS,
            optional_columns=(
                TechniquesContentColumns.NODE_ID,
                TechniquesContentColumns.DESCRIPTION,
                TechniquesContentColumns.ANNOTATION,
                TechniquesContentColumns.PARENT_REF_ID,
                TechniquesContentColumns.TACTIC_REF_IDS,
                TechniquesContentColumns.GROUPS,
                TechniquesContentColumns.REFERENCE_CONTROLS,
            ),
            translatable_columns=(
                TechniquesContentColumns.NAME,
                TechniquesContentColumns.DESCRIPTION,
        ...
        MetaTypes.REFERENCE_CONTROLS: ContentSheetSchema(
            ...
            required_columns=(
                ReferenceControlsContentColumns.REF_ID,
                ReferenceControlsContentColumns.NAME,
            ),
            column_constraints=REFERENCE_CONTROLS_CONTENT_COLUMN_CONSTRAINTS,
            optional_columns=(
                ReferenceControlsContentColumns.CATEGORY,
                ReferenceControlsContentColumns.CSF_FUNCTION,
                ReferenceControlsContentColumns.DESCRIPTION,
        ...
        MetaTypes.RISK_MATRIX: ContentSheetSchema(
            ...
            required_columns=(
                RiskMatrixContentColumns.TYPE,
                RiskMatrixContentColumns.ID,
                RiskMatrixContentColumns.ABBREVIATION,
                RiskMatrixContentColumns.NAME,
                RiskMatrixContentColumns.DESCRIPTION,
            ),
        ...
        MetaTypes.IMPLEMENTATION_GROUPS: ContentSheetSchema(
            ...
            required_columns=(
                ImplementationGroupsContentColumns.REF_ID,
                ImplementationGroupsContentColumns.NAME,
            ),
            column_constraints=IMPLEMENTATION_GROUPS_CONTENT_COLUMN_CONSTRAINTS,
            optional_columns=(
                ImplementationGroupsContentColumns.DESCRIPTION,
                ImplementationGroupsContentColumns.DEFAULT_SELECTED,
            ),
            translatable_columns=(
                ImplementationGroupsContentColumns.NAME,
                ImplementationGroupsContentColumns.DESCRIPTION,
        ...
        MetaTypes.REQUIREMENT_MAPPING_SET: ContentSheetSchema(
            ...
            required_columns=(
                RequirementMappingSetContentColumns.SOURCE_NODE_ID,
                RequirementMappingSetContentColumns.TARGET_NODE_ID,
                RequirementMappingSetContentColumns.RELATIONSHIP,
            ),
            column_constraints=REQUIREMENT_MAPPING_SET_CONTENT_COLUMN_CONSTRAINTS,
            optional_columns=(
                RequirementMappingSetContentColumns.RATIONALE,
                RequirementMappingSetContentColumns.STRENGTH_OF_RELATIONSHIP,
            ),
        ),
        MetaTypes.SCORES: ContentSheetSchema(
            ...
            optional_columns=(
                ScoresContentColumns.DESCRIPTION,
                ScoresContentColumns.DESCRIPTION_DOC,
            ),
            translatable_columns=(
                ScoresContentColumns.NAME,
                ScoresContentColumns.DESCRIPTION,
                ScoresContentColumns.DESCRIPTION_DOC,
            ),
        ),
        MetaTypes.ANSWERS: ContentSheetSchema(
            ...
            required_columns=(
                AnswersContentColumns.ID,
                AnswersContentColumns.QUESTION_TYPE,
            ),
            column_constraints=ANSWERS_CONTENT_COLUMN_CONSTRAINTS,
            optional_columns=(
                AnswersContentColumns.QUESTION_CHOICES,
                AnswersContentColumns.DESCRIPTION,
                AnswersContentColumns.SELECT_IMPLEMENTATION_GROUPS,
                AnswersContentColumns.ADD_SCORE,
                AnswersContentColumns.COMPUTE_RESULT,
                AnswersContentColumns.COLOR,
            ),
            translatable_columns=(
                AnswersContentColumns.QUESTION_CHOICES,
                AnswersContentColumns.DESCRIPTION,
        ...
        MetaTypes.URN_PREFIX: ContentSheetSchema(
            ...
            required_columns=(
                URNPrefixContentColumns.PREFIX_ID,
                URNPrefixContentColumns.PREFIX_VALUE,
            ),
...
class URNMetadataFormat(Enum):
    ...
    # the catalog ref_id is appended by the converter, so the base stops here
    TTP_CATALOG_BASE_URN = f"{URNObjects.URN_BEGGINING.value}:{PACKAGER_INDICATOR}:{URNObjects.URN_3RD_WORD.value}:{URNObjects.TTP_CATALOG.value}"
    TTP_CATALOG_URN = f"{TTP_CATALOG_BASE_URN}:{ID_INDICATOR}"
    TACTICS_BASE_URN = f"{URNObjects.URN_BEGGINING.value}:{PACKAGER_INDICATOR}:{URNObjects.URN_3RD_WORD.value}:{URNObjects.TACTIC.value}:{ID_INDICATOR}"
    TECHNIQUES_BASE_URN = f"{URNObjects.URN_BEGGINING.value}:{PACKAGER_INDICATOR}:{URNObjects.URN_3RD_WORD.value}:{URNObjects.TECHNIQUE.value}:{ID_INDICATOR}"
...
def enum_sequence_to_strings(values: Sequence[Enum | str]) -> tuple[str, ...]:
    return tuple(
        str(value.value) if isinstance(value, Enum) else value for value in values
    )
...
def check_file_validity(
    ...
    for f in files:
        if not os.path.exists(f):
            raise ValueError(
                f'({fct_name}) {(f"[{file_context}] " if file_context else "")}"{f}" doesn\'t exist'
            )

        if not os.path.isfile(f):
            raise ValueError(
                f'({fct_name}) {(f"[{file_context}] " if file_context else "")}"{f}" isn\'t a file'
            )

        if valid_file_extensions and not os.path.basename(f).endswith(
            valid_file_extensions
        ):
            raise ValueError(
                f'({fct_name}) {(f"[{file_context}] " if file_context else "")}"{f}" isn\'t a valid {filetype_name} file'
                f"\n> 💡 Valid {filetype_name} file formats : "
                + ", ".join(f"{f}" for f in valid_file_extensions)
            )
...
def get_meta_sheets_with_type(wb: Workbook, context: str) -> Dict[str, str]:
    ...
    for sheet_name in wb.sheetnames:
        ...
        type_value = get_meta_value(
            df, MandatoryMetaKeys.TYPE, sheet_name, context=context
        )
...
def get_meta_sheets_names_from_type(
    wb: Workbook, sheet_type: MetaTypes, context: str
) -> List[str]:
...
def get_meta_value(
    ...
    if df.shape[1] == 0:
        if required:
            raise ValueError(
                f'{context_prefix}[{sheet_name}] Missing required key "{key_name}" because the meta sheet is empty.'
            )
    ...
    if matches.empty:
        if required:
            raise ValueError(
                f'{context_prefix}[{sheet_name}] Missing required key "{key_name}" in meta sheet.'
            )
        return (None, None) if with_row else None
    ...
    value = (
        matches.iloc[0, 1] if matches.shape[1] > 1 else None
    )  # Return "None" if 2nd column is empty
    row = matches.index[0] + 1  # Excel-style row number

    if pd.isna(value) or str(value).strip() == "":
        if required:
            raise ValueError(
                f'{context_prefix}[{sheet_name}] Row #{row}: Key "{key_name}" is present but has an empty value.'
            )
...
def get_non_empty_column_values(
    df: pd.DataFrame, column_name: ContentColumn | str
) -> List[str]:
...
def validate_urn_type(
    urn: str, urn_type: URNMetadataFormat, context: str, row: str | int = None
):
...
def validate_urn(urn: str, context: str = None, row: str | int = None):
    ...
    if not re.fullmatch(pattern, urn):
        raise ValueError(
            f"({context if context else 'validate_urn'}){' Row #' + str(row) + ':' if row else ''} Invalid URN \"{urn}\" : Only lowercase alphanumeric characters, '-', '_', and '.' are allowed. URNs must begin with \"urn:\""
        )
...
def validate_ref_id(ref_id: str, context: str = None, row: str | int = None):
    if not re.fullmatch(r"[a-zA-Z0-9._-]+", ref_id):
        raise ValueError(
            f"({context if context else 'validate_ref_id'}){' Row #' + str(row) + ':' if row else ''} Invalid Ref. ID \"{ref_id}\" : Only alphanumeric characters, '-', '_', and '.' are allowed"
        )


def validate_ref_id_with_spaces(
    ref_id: str, context: str = None, row: str | int = None
):
    if not re.fullmatch(r"[a-zA-Z0-9._\- ]+", ref_id):
        raise ValueError(
            f"({context if context else 'validate_ref_id'}){' Row #' + str(row) + ':' if row else ''} Invalid Ref. ID \"{ref_id}\" : Only alphanumeric characters, '-', '_', ' ', and '.' are allowed"
...
def validate_sheet_name(sheet_name: str, context: str = None):
    if not (
        sheet_name.endswith(SheetTypes.META.value)
        or sheet_name.endswith(SheetTypes.CONTENT.value)
    ):
        raise ValueError(
            f"({context if context else 'validate_sheet_name'}) Invalid sheet name \"{sheet_name}\". Sheet names must end with '{SheetTypes.META.value}' or '{SheetTypes.CONTENT.value}'"
        )
...
def validate_no_spaces(
    value: str, value_name: str, context: str = None, row: int = None
):
    if " " in str(value):
        raise ValueError(
            f'({context if context else "validate_no_spaces"}){" Row #" + str(row) + ":" if row is not None else ""} Invalid value for "{value_name}": Spaces are not allowed (got "{value}")'
        )


def print_sheet_validation(
    sheet_name: str, verbose: bool = False, ctx: ConsoleContext = None
):
    ...
    if not ctx:
        ...
        if verbose:
            print(
                f"ℹ️   [INFO] Check for warning / verbose messages in the console, if any"
            )
        else:
        ...
        if verbose:
            if sheet_warnings:
                if sheet_verbose:
                    print(
                        f'🟣 [CHECK] Valid sheet with warnings and verbose messages : "{sheet_name}" (Warn: {len(sheet_warnings)} / Verb: {len(sheet_verbose)})'
                    )
                else:
                    print(
                        f'🟡 [CHECK] Valid sheet with warnings: "{sheet_name}" (Warn: {len(sheet_warnings)} / Verb: 0)'
                    )
            else:
                if sheet_verbose:
                    print(
                        f'🔵 [CHECK] Valid sheet with verbose messages : "{sheet_name}" (Warn: 0 / Verb: {len(sheet_verbose)})'
                    )
                else:
                    print(f'🟢 [CHECK] Valid sheet: "{sheet_name}"')
        else:
            if sheet_warnings:
                print(
                    f'🟡 [CHECK] Valid sheet with warnings: "{sheet_name}" (Warn: {len(sheet_warnings)})'
                )
            else:
                print(f'🟢 [CHECK] Valid sheet: "{sheet_name}"')
...
def validate_labels(labels_value: str, context: str, row: int):
    ...
    raw_labels = [
        x
        for x in re.split(
            CommonSeparatorRegex.SPACE_COMMA_LF.value, str(labels_value).strip()
        )
        if x
    ]
...
def validate_integer_value(
    ...
    # --- [OR] validate single value ---
        ...
        entries = (
            re.split(split_regex, str(value_or_df)) if split_regex else [value_or_df]
        )
        values = []
        for cell_line, entry in enumerate(entries, start=1):
    ...
    if invalid_values:
        ...
        for v, r, cell_line in invalid_values:
            if r is not None:
                cell_line_details = (
                    f" (Cell element #{cell_line})" if cell_line is not None else ""
                )
...
def validate_meta_sheet(
    df: pd.DataFrame,
    sheet_name: str,
    expected_keys: Sequence[MetaKey | str] | None,
    expected_type: MetaTypes,
    context: str,
):
    ...
    if expected_keys:
        expected_keys = tuple(
            key.value if isinstance(key, MetaKey) else key for key in expected_keys
        )
        for key in expected_keys:
            get_meta_value(df, key, sheet_name, required=True, context=context)

    # Validate presence and value of "type" key
    type_value, type_row = get_meta_value(
        df,
        MandatoryMetaKeys.TYPE,
        sheet_name,
        required=True,
        with_row=True,
        context=context,
    ...
    if type_value != expected_type.value:
        raise ValueError(
            f'({context}) [{sheet_name}] Row #{type_row}: Invalid type "{type_value}". Expected "{expected_type.value}"'
        )


def validate_optional_values_meta_sheet(
    df: pd.DataFrame,
    sheet_name: str,
    optional_keys: Sequence[MetaKey | str],
    context: str,
    verbose: bool = False,
    ...
    optional_keys = tuple(
        key.value if isinstance(key, MetaKey) else key for key in optional_keys
    )
    ...
    for key in optional_keys:
        ...
        if row is not None:
            if value is None:
                raise ValueError(
                    f'({context}) [{sheet_name}] Row #{row}: Optional key "{key}" is present but has no value'
...
def validate_extra_locales_in_meta(
    df: pd.DataFrame,
    sheet_name: str,
    translatable_keys: Sequence[MetaKey | str],
    context: str,
    ctx: ConsoleContext = None,
):

    translatable_keys = tuple(
        key.value if isinstance(key, MetaKey) else key for key in translatable_keys
    )
...
def validate_related_content_sheet_from_name_key(
    wb: Workbook,
    df: pd.DataFrame,
    sheet_name: str,
    name_key: MetaKey | str,
    context: str,
):

    value, row = get_meta_value(
        df, name_key, sheet_name, with_row=True, context=context
    )
    if value is None:
...
def _framework_validate_definition_keys(
    wb: Workbook,
    df: pd.DataFrame,
    sheet_name: str,
    definition_keys: Sequence[FrameworkMetaKeys | str],
):
    ...
    for def_key in definition_keys:
        value, row = get_meta_value(
            df, def_key, sheet_name, with_row=True, context=fct_name
        )
        if value is None:
            continue
...
def _framework_validate_meta_min_max_score(df: pd.DataFrame, sheet_name: str):
    ...
    min_score, min_score_row = get_meta_value(
        df, FrameworkMetaKeys.MIN_SCORE, sheet_name, with_row=True, context=fct_name
    )
    max_score, max_score_row = get_meta_value(
        df, FrameworkMetaKeys.MAX_SCORE, sheet_name, with_row=True, context=fct_name
    )
    ...
    if min_score is None or max_score is None:
        missing_key = (
            FrameworkMetaKeys.MIN_SCORE.value
            if min_score is None
            else FrameworkMetaKeys.MAX_SCORE.value
        )
        present_key = (
            FrameworkMetaKeys.MAX_SCORE.value
            if min_score is None
            else FrameworkMetaKeys.MIN_SCORE.value
        )
        raise ValueError(
    ...
    validate_integer_value(
        ...
        context=fct_name,
        row=min_score_row,
        value_name=FrameworkMetaKeys.MIN_SCORE.value,
        min=0,
    )
    validate_integer_value(
        max_score,
        ...
        context=fct_name,
        row=max_score_row,
        value_name=FrameworkMetaKeys.MAX_SCORE.value,
        min=0,
    )
    ...
    if min_score > max_score:
        raise ValueError(
            f'({fct_name}) [{sheet_name}] Invalid score range: "{FrameworkMetaKeys.MIN_SCORE.value}" ({min_score}) must be less than or equal to "{FrameworkMetaKeys.MAX_SCORE.value}" ({max_score})'
        )


# [META] Library {OK}²
def validate_library_meta(
    df: pd.DataFrame, sheet_name: str, verbose: bool = False, ctx: ConsoleContext = None
):
    ...
    validate_optional_values_meta_sheet(
        df, sheet_name, schema.optional_keys, fct_name, verbose, ctx
    )

    # URN
    urn_value, urn_row = get_meta_value(
        df,
        LibraryMetaKeys.URN,
        sheet_name,
        required=True,
    ...
    # ref_id
    ref_id_value, ref_id_row = get_meta_value(
        df,
        LibraryMetaKeys.REF_ID,
        sheet_name,
        required=True,
    ...
    # version
    version_value, version_row = get_meta_value(
        df,
        LibraryMetaKeys.VERSION,
        sheet_name,
        required=True,
    ...
    validate_integer_value(
        ...
        context=fct_name,
        row=version_row,
        value_name=LibraryMetaKeys.VERSION.value,
        positive_only=True,
    )

    # labels (Optional)
    labels_value, labels_row = get_meta_value(
        df,
        LibraryMetaKeys.LABELS,
        sheet_name,
        required=False,
        with_row=True,
        context=fct_name,
    ...
    # locale
    locale_value, locale_row = get_meta_value(
        df,
        LibraryMetaKeys.LOCALE,
        sheet_name,
        required=True,
    ...
    # Extra locales
    validate_extra_locales_in_meta(
        df, sheet_name, schema.translatable_keys, fct_name, ctx
    )
...
def validate_framework_meta(
    ...
    # base_urn
    base_urn_value, base_urn_row = get_meta_value(
        df,
        FrameworkMetaKeys.BASE_URN,
        sheet_name,
        required=True,
    ...
    validate_urn_type(
        base_urn_value, URNMetadataFormat.FRAMEWORK_BASE_URN, fct_name, base_urn_row
    )
...
def validate_threats_meta(
    ...
    validate_urn(base_urn_value, fct_name, base_urn_row)
    validate_urn_type(
        base_urn_value, URNMetadataFormat.THREATS_BASE_URN, fct_name, base_urn_row
    )
...
def validate_ttp_catalog_meta(
    ...
    expected_type = MetaTypes.TTP_CATALOG
    schema = META_SHEET_SCHEMAS[expected_type]
    ...
    validate_urn(base_urn_value, fct_name, base_urn_row)
    validate_urn_type(
        base_urn_value, URNMetadataFormat.TTP_CATALOG_BASE_URN, fct_name, base_urn_row
    )

    # grouping_definition points at a "<name>_content" sheet, like implementation groups do
    validate_related_content_sheet_from_name_key(
        wb, df, sheet_name, TTPCatalogMetaKeys.GROUPING_DEFINITION, fct_name
    )
...
def validate_ttp_groups_meta(
    ...
    expected_type = MetaTypes.TTP_GROUPS
    schema = META_SHEET_SCHEMAS[expected_type]
    ...
    # name
    validate_related_content_sheet_from_name_key(
        wb, df, sheet_name, TTPGroupsMetaKeys.NAME, fct_name
    )
...
# [META] Tactics
def validate_tactics_meta(
    df: pd.DataFrame, sheet_name: str, verbose: bool = False, ctx: ConsoleContext = None
):

    fct_name = get_current_fct_name()

    expected_type = MetaTypes.TACTICS
    schema = META_SHEET_SCHEMAS[expected_type]
    ...
    validate_urn(base_urn_value, fct_name, base_urn_row)
    validate_urn_type(
        base_urn_value, URNMetadataFormat.TACTICS_BASE_URN, fct_name, base_urn_row
    )

    # catalog_urn
    catalog_urn_value, catalog_urn_row = get_meta_value(
        df,
        TacticsMetaKeys.CATALOG_URN,
        sheet_name,
        required=True,
        with_row=True,
        context=fct_name,
    )
    validate_urn(catalog_urn_value, fct_name, catalog_urn_row)
    validate_urn_type(
        catalog_urn_value, URNMetadataFormat.TTP_CATALOG_URN, fct_name, catalog_urn_row
    )
...
# [META] Techniques
def validate_techniques_meta(
    df: pd.DataFrame, sheet_name: str, verbose: bool = False, ctx: ConsoleContext = None
):

    fct_name = get_current_fct_name()

    expected_type = MetaTypes.TECHNIQUES
    schema = META_SHEET_SCHEMAS[expected_type]
    ...
    validate_urn(base_urn_value, fct_name, base_urn_row)
    validate_urn_type(
        base_urn_value, URNMetadataFormat.TECHNIQUES_BASE_URN, fct_name, base_urn_row
    )

    # catalog_urn
    catalog_urn_value, catalog_urn_row = get_meta_value(
        df, TechniquesMetaKeys.CATALOG_URN, sheet_name, with_row=True, context=fct_name
    )
    if catalog_urn_value is not None:
        validate_urn(catalog_urn_value, fct_name, catalog_urn_row)
        validate_urn_type(
            catalog_urn_value,
            URNMetadataFormat.TTP_CATALOG_URN,
            fct_name,
            catalog_urn_row,
        )

    # tactics_base_urn
    tactics_base_urn_value, tactics_base_urn_row = get_meta_value(
        df,
        TechniquesMetaKeys.TACTICS_BASE_URN,
        sheet_name,
        with_row=True,
    ...
    if tactics_base_urn_value is not None:
        validate_urn(tactics_base_urn_value, fct_name, tactics_base_urn_row)
        validate_urn_type(
            tactics_base_urn_value,
            URNMetadataFormat.TACTICS_BASE_URN,
            fct_name,
            tactics_base_urn_row,
        )
...
def validate_reference_controls_meta(
    df: pd.DataFrame, sheet_name: str, verbose: bool = False, ctx: ConsoleContext = None
):
    ...
    base_urn_value, base_urn_row = get_meta_value(
        df,
        ReferenceControlsMetaKeys.BASE_URN,
        sheet_name,
        required=True,
        with_row=True,
        context=fct_name,
    ...
    validate_urn_type(
        base_urn_value,
        URNMetadataFormat.REFERENCE_CONTROLS_BASE_URN,
        fct_name,
        base_urn_row,
    )
...
def validate_implementation_groups_meta(
    wb: Workbook,
    df: pd.DataFrame,
    sheet_name: str,
    verbose: bool = False,
    ...
    # name
    validate_related_content_sheet_from_name_key(
        wb, df, sheet_name, ImplementationGroupsMetaKeys.NAME, fct_name
    )
...
# [META] Mappings {OK}²
def validate_requirement_mapping_set_meta(
    df: pd.DataFrame, sheet_name: str, verbose: bool, ctx: ConsoleContext = None
):
    ...
    source_framework_urn_value, source_framework_urn_row = get_meta_value(
        df,
        RequirementMappingSetMetaKeys.SOURCE_FRAMEWORK_URN,
        sheet_name,
        required=True,
    ...
    validate_urn_type(
        source_framework_urn_value,
        URNMetadataFormat.MAPPING_SOURCE_AND_TARGET_FRAMEWORK_URN,
        fct_name,
        source_framework_urn_row,
    )

    # target_framework_urn
    target_framework_urn_value, target_framework_urn_row = get_meta_value(
        df,
        RequirementMappingSetMetaKeys.TARGET_FRAMEWORK_URN,
        sheet_name,
        required=True,
    ...
    validate_urn_type(
        target_framework_urn_value,
        URNMetadataFormat.MAPPING_SOURCE_AND_TARGET_FRAMEWORK_URN,
        fct_name,
        target_framework_urn_row,
    )

    # source_node_base_urn
    source_node_base_urn_value, source_node_base_urn_row = get_meta_value(
        df,
        RequirementMappingSetMetaKeys.SOURCE_NODE_BASE_URN,
        sheet_name,
        required=True,
    ...
    validate_urn_type(
        source_node_base_urn_value,
        URNMetadataFormat.MAPPING_SOURCE_AND_TARGET_NODE_BASE_URN,
        fct_name,
        source_node_base_urn_row,
    )

    # target_node_base_urn
    target_node_base_urn_value, target_node_base_urn_row = get_meta_value(
        df,
        RequirementMappingSetMetaKeys.TARGET_NODE_BASE_URN,
        sheet_name,
        required=True,
    ...
    validate_urn_type(
        target_node_base_urn_value,
        URNMetadataFormat.MAPPING_SOURCE_AND_TARGET_NODE_BASE_URN,
        fct_name,
        target_node_base_urn_row,
    )
    ...
    for key in keys_to_check_no_spaces:
        value, row = get_meta_value(
            df, key, sheet_name, required=True, with_row=True, context=fct_name
        )
...
def validate_scores_meta(
    ...
    # name
    validate_related_content_sheet_from_name_key(
        wb, df, sheet_name, ScoresMetaKeys.NAME, fct_name
    )
...
def validate_answers_meta(
    ...
    # name
    validate_related_content_sheet_from_name_key(
        wb, df, sheet_name, AnswersMetaKeys.NAME, fct_name
    )
...
# Global Checks
def validate_content_sheet(
    df: pd.DataFrame,
    sheet_name: str,
    required_columns: Sequence[ContentColumn | str],
    context: str,
):
    ...
    reference_id_columns = {
        FrameworkContentColumns.REF_ID.value,
        ThreatsContentColumns.REF_ID.value,
        ReferenceControlsContentColumns.REF_ID.value,
        RiskMatrixContentColumns.ID.value,
        ImplementationGroupsContentColumns.REF_ID.value,
        AnswersContentColumns.ID.value,
    }
    ...
    if required_columns:
        ...
        for col in required_columns:
            if col not in df.columns:
                raise ValueError(
                    f'({context}) [{sheet_name}] Missing required column "{col}" in content sheet'
                )
        ...
        for col in required_columns:
            ...
            if required_values_missing:
                raise ValueError(
                    ...
                    + "\n   - ".join(
                        f"Row #{idx + 2}" for idx in required_values_missing
                    )
                )
            ...
            if invalid_ref_ids:
                raise ValueError(
                    ...
                    + "\n   - ".join(
                        f"Row #{idx + 2}: {value}" for value, idx in invalid_ref_ids
                    )
                )


def validate_optional_columns_content_sheet(
    df: pd.DataFrame,
    sheet_name: str,
    optional_columns: Sequence[ContentColumn | str],
    context: str,
    verbose: bool = False,
    ctx: ConsoleContext = None,
):
    ...
    for col in optional_columns:
        ...
        is_entirely_empty = all(
            pd.isna(val) or str(val).strip() == "" for val in df[col]
        )
...
def validate_columns_presence_together(
    df: pd.DataFrame,
    column_names: Sequence[ContentColumn | str],
    sheet_name: str,
    context: str = None,
):
...
def validate_column_max_length(
    df: pd.DataFrame,
    ...
    if values_exceeding_limit:
        raise ValueError(
            ...
            + "\n   - ".join(
                f"Row #{row}: {length} characters"
                for row, length in values_exceeding_limit
            )
...
def validate_unique_column_values(
    df: pd.DataFrame,
    column_names: Sequence[ContentColumn | str],
    sheet_name: str,
    context: str = None,
    ...
    for column_name in column_names:
        if column_name not in df.columns:
            raise ValueError(
                f'({context}) [{sheet_name}] Column "{column_name}" not found in sheet'
            )
        ...
        duplicates = filtered_df[column_name][
            filtered_df[column_name].duplicated(keep=False)
        ]

        if not duplicates.empty:
            duplicate_rows = (
                duplicates.index + 2
            )  # Excel-like row number (1-based + header)
...
def validate_extra_locales_in_content(
    df: pd.DataFrame,
    sheet_name: str,
    context: str,
    ctx: ConsoleContext = None,
    ...
    for col in df.columns:
        ...
        non_empty_found = any(
            pd.notna(val) and str(val).strip() != "" for val in df[col]
        )
        ...
        content_sheet_type = (
            get_content_sheet_type(wb, sheet_name, context) if wb is not None else None
        )
        ...
        if (
            content_sheet_type == MetaTypes.FRAMEWORK.value
            and base_col == FrameworkContentColumns.QUESTIONS.value
        ):
            questions_constraints = CONTENT_SHEET_SCHEMAS[
                MetaTypes.FRAMEWORK
            ].column_constraints[FrameworkContentColumns.QUESTIONS]
            validate_cell_line_count_alignment(
                df,
                base_col,
                col,
                sheet_name,
        ...
        elif content_sheet_type == MetaTypes.ANSWERS.value and base_col in (
            AnswersContentColumns.QUESTION_CHOICES.value,
            AnswersContentColumns.DESCRIPTION.value,
        ):
            answer_column = AnswersContentColumns(base_col)
            answer_constraints = CONTENT_SHEET_SCHEMAS[
                MetaTypes.ANSWERS
            ].column_constraints[answer_column]
            validate_cell_line_count_alignment(
                df,
                base_col,
                col,
...
def get_content_sheet_base_name(content_sheet_name: str) -> str:
    if not content_sheet_name.endswith(SheetTypes.CONTENT.value):
        raise ValueError(
            f'Invalid sheet name: "{content_sheet_name}" does not end with "{SheetTypes.CONTENT.value}"'
        )
...
def get_content_sheet_type(wb: Workbook, content_sheet_name: str, context: str) -> str:
    ...
    if meta_sheet_name not in meta_sheets_with_type:
        raise ValueError(
            f'({context}) [{meta_sheet_name}] Missing or empty "{MandatoryMetaKeys.TYPE.value}" field in meta sheet'
        )
...
def get_corresponding_type_sheet_names(
    sheet_names: List[str], sheet_type: SheetTypes
) -> List[str]:
    ...
    for name in sheet_names:
        ...
        if not matched:
            raise ValueError(
                f'Invalid sheet name: "{name}" does not end with a known sheet type suffix {suffixes}'
            )
...
def check_content_sheet_usage_in_frameworks(
    wb: Workbook,
    sheet_name: str,
    meta_field: FrameworkMetaKeys | str,
    fct_name: str,
    ctx: ConsoleContext = None,
) -> List[str]:
    """
    ...
    if frameworks_with_reference:
        print(
            f"ℹ️  [INFO] ({fct_name}) [{sheet_name}] Sheet referenced by the sheet(s): {', '.join(f'"{s}"' for s in frameworks_with_reference)}"
        )
    else:
        warn_msg = (
...
def check_unused_ids_in_frameworks(
    wb: Workbook,
    df_ids: pd.DataFrame,
    id_column: ContentColumn | str,
    target_column: ContentColumn | str,
    frameworks_sheet_names: List[str],
    sheet_name: str,
    context: str,
    ctx: ConsoleContext = None,
    verbose: bool = False,
            ...
            for cell in df_fw[target_column].dropna().astype(str):
                ...
                entries = [
                    ...
                    for entry in re.split(
                        CommonSeparatorRegex.COMMA_LF.value, str(cell)
                    )
                    if entry.strip()
                ]
...
def _implementation_groups_check_unused_default_ids_in_frameworks(
    wb: Workbook, df: pd.DataFrame, frameworks_sheet_names: List[str], sheet_name: str
):
    ...
    default_selected_mask = (
        df[ImplementationGroupsContentColumns.DEFAULT_SELECTED.value]
        .fillna("")
        .astype(str)
        .str.strip()
        .isin(["x", "X"])
    ...
    default_selected_ids = get_non_empty_column_values(
        default_selected_df, ImplementationGroupsContentColumns.REF_ID
    )

    if not default_selected_ids:
        return

    unused_default_ids = check_unused_ids_in_frameworks(
        wb,
        default_selected_df,
        ImplementationGroupsContentColumns.REF_ID,
        FrameworkContentColumns.IMPLEMENTATION_GROUPS,
        frameworks_sheet_names,
        sheet_name,
        fct_name,
        emit_messages=False,
    )
...
def validate_allowed_column_values(
    ...
    allowed_values_tip = (
        f'Allowed pattern is: "{allowed_pattern}"'
        if allowed_pattern is not None
        else f"Allowed values are: {', '.join(f'"{v}"' for v in allowed_values)}"
    )
    ...
    if not split_regex:
        ...
        invalid_mask = (
            ~cleaned_series.map(
                lambda value: bool(re.fullmatch(allowed_pattern, value))
            )
            if allowed_pattern is not None
            else ~cleaned_series.isin(allowed_values)
        )
        ...
        if invalid_mask.any():
            ...
            details = "\n   - " + "\n   - ".join(
                f'Row #{idx + 2} : "{val}"' for idx, val in invalid_series.items()
            )
    ...
    for idx, cell_value in df[column_name].dropna().items():
        ...
        for i, entry in enumerate(entries, start=1):
            if (
                allowed_pattern is not None and not re.fullmatch(allowed_pattern, entry)
            ) or (allowed_pattern is None and entry not in allowed_values):
...
def validate_cell_line_count_alignment(
    ...
    for idx, row in df.iterrows():
        ...
        if cmp_count not in valid_cmp_counts:
            expected_count = (
                f"1 or {ref_count}"
                if allow_single_cmp and ref_count != 1
                else str(ref_count)
            )
            errors.append(
...
def _URN_prefix_check_unused_ids_in_frameworks(
    wb: Workbook,
    df_ids: pd.DataFrame,
    frameworks_sheet_names: List[str],
    sheet_name: str,
    ...
    target_columns = [
        FrameworkContentColumns.THREATS.value,
        FrameworkContentColumns.REFERENCE_CONTROLS.value,
    ]
            ...
            for target_column in target_columns:
                ...
                for cell in df_fw[target_column]:
                    if pd.isna(cell):
                        continue
                    entries = [
                        entry.strip() for entry in str(cell).split(",") if entry.strip()
                    ]
                    prefix_parts = [
    ...
    if unused_ids:
        ...
        msg = f'💬 ℹ️  [INFO] ({context}) [{sheet_name}] All Prefix ID(s) from column "{id_column}" are used in framework sheets'
        print(msg)
        if ctx:
            ctx.add_sheet_verbose_msg(sheet_name, msg)
...
def _URN_prefix_validate_ids_usage_in_frameworks(
    wb: Workbook,
    df: pd.DataFrame,
    sheet_name: str,
    ctx: ConsoleContext = None,
    ...
    framework_sheets = get_meta_sheets_names_from_type(
        wb, MetaTypes.FRAMEWORK, fct_name
    )
    framework_sheets = get_corresponding_type_sheet_names(
        framework_sheets, SheetTypes.CONTENT
    )

    # 2. Check if every Prefix IDs are actually used in "framework" sheets
    if framework_sheets:
        _URN_prefix_check_unused_ids_in_frameworks(
            wb, df, framework_sheets, sheet_name, fct_name, ctx, verbose
        )
        return
...
def _URN_prefix_classify_prefix_usage(
    wb: Workbook,
    df_urn_prefix: pd.DataFrame,
    meta_sheets: List[str],
    meta_type: MetaTypes,
    sheet_name: str,
    fct_name: str,
    ctx: ConsoleContext = None,
    ...
    if meta_type == MetaTypes.THREATS:
        ...
        raise ValueError(
            f"({fct_name}) [{sheet_name}] Unsupported Meta type: {meta_type}"
        )

    prefix_values = (
        df_urn_prefix[URNPrefixContentColumns.PREFIX_VALUE.value]
        .dropna()
        .astype(str)
        .str.strip()
        .unique()
...
def _URN_prefix_validate_prefix_values_and_dependencies(
    wb: Workbook,
    df: pd.DataFrame,
    sheet_name: str,
    ctx: ConsoleContext = None,
    ...
    ref_ctrl_sheets = get_meta_sheets_names_from_type(
        wb, MetaTypes.REFERENCE_CONTROLS, fct_name
    )
    ...
    if threats_sheets:
        internal_threats, external_threats, _ = _URN_prefix_classify_prefix_usage(
            wb, df, threats_sheets, MetaTypes.THREATS, sheet_name, fct_name, ctx
        )
    if ref_ctrl_sheets:
        internal_ref_ctrl, external_ref_ctrl, _ = _URN_prefix_classify_prefix_usage(
            wb,
            df,
            ref_ctrl_sheets,
            MetaTypes.REFERENCE_CONTROLS,
            sheet_name,
            fct_name,
            ctx,
        )
    ...
    required_dependencies = sorted(
        set(normalized_ext_threats + normalized_ext_ref_ctrl)
    )
    ...
    if required_dependencies:
        ...
        try:
            ...
            raise ValueError(
                f'({fct_name}) [{sheet_name}] Could not read "{MandatorySheets.LIBRARY_META.value}" sheet: {e}'
            )
        ...
        declared_dependencies = [
            dep.strip()
            for dep in meta_dict.get("dependencies", "").split(",")
            if dep.strip()
        ]

        # 6. Compare with required dependencies
        missing_dependencies = [
            dep for dep in required_dependencies if dep not in declared_dependencies
        ]
...
def print_info_about_internal_external_URN_prefix(
    sheet_name: str,
    internal_threats: List[str],
    external_threats: List[str],
    internal_ref_ctrl: List[str],
    external_ref_ctrl: List[str],
    context: str = None,
    verbose: bool = False,
    ctx: ConsoleContext = None,
...
def _req_map_set_validate_unique_mappings(
    df: pd.DataFrame,
    sheet_name: str,
    warn_only: bool = False,
    ctx: ConsoleContext = None,
    ...
    if source_column not in df.columns or target_column not in df.columns:
        raise ValueError(
            f'({fct_name}) [{sheet_name}] Columns "{source_column}" and/or "{target_column}" not found'
        )
    ...
    df_clean = df_clean[
        (df_clean[source_column] != "") & (df_clean[target_column] != "")
    ]

    duplicates = df_clean[
        df_clean.duplicated(subset=[source_column, target_column], keep=False)
    ]
...
def _req_map_set_validate_mapping_node_ids_against_sheets(
    wb: Workbook,
    df: pd.DataFrame,
    sheet_name: str,
    fct_name: str,
    ...
    used_source_ids = [
        str(val).split(":")[-1]
        for val in df[RequirementMappingSetContentColumns.SOURCE_NODE_ID.value].dropna()
    ]
    used_target_ids = [
        str(val).split(":")[-1]
        for val in df[RequirementMappingSetContentColumns.TARGET_NODE_ID.value].dropna()
    ]

    source_missing_counts = Counter(
        node_id
        for node_id in used_source_ids
        if source_sheet_available and node_id not in source_ids
    )
    target_missing_counts = Counter(
        node_id
        for node_id in used_target_ids
    ...
    total_missing_sources = (
        "???" if not source_sheet_available else sum(source_missing_counts.values())
    )
    total_missing_targets = (
        "???" if not target_sheet_available else sum(target_missing_counts.values())
    )
    ...
    for idx, row in df.iterrows():
        ...
        if ref_id:
            try:
                validate_ref_id(
                    ref_id, fct_name, idx
                )  # _with_spaces(ref_id, fct_name, idx)
            except Exception as e:
...
def _framework_validate_column_against_reference_sheet(
    wb: Workbook,
    df: pd.DataFrame,
    column_name: FrameworkContentColumns | str,
    ...
    if column_name not in column_to_key_mapping:
        raise ValueError(
            f'({context}) [{current_sheet_name}] Unsupported column "{column_name}" for this validation'
        )

    # Get associated meta sheet
    meta_sheet_name = get_corresponding_type_sheet_names(
        [current_sheet_name], SheetTypes.META
    )[0]
    if meta_sheet_name not in wb.sheetnames:
        raise ValueError(
            f'({context}) [{current_sheet_name}] Missing meta sheet "{meta_sheet_name}" required to validate column "{column_name}"'
        )
    ...
    if ref_content_sheet not in wb.sheetnames:
        raise ValueError(
            f'({context}) [{current_sheet_name}] Referenced sheet "{ref_content_sheet}" (from key "{meta_key.value}") not found'
        )
    ...
    if column_name == FrameworkContentColumns.IMPLEMENTATION_GROUPS.value:
        ...
        raise RuntimeError(
            f"({context}) [{current_sheet_name}] Unexpected internal error: invalid column dispatch"
        )

    if ref_column not in ref_df.columns:
        raise ValueError(
            f'({context}) [{current_sheet_name}] Referenced sheet "{ref_content_sheet}" does not contain required column "{ref_column}"'
        )
    ...
    if invalid_values:
        raise ValueError(
            ...
            + "\n   - ".join(
                f"Row #{idx + 2} (element #{i}) -> {item}"
                for idx, item, i in invalid_values
            )
...
def _framework_validate_framework_column_urns(
    wb: Workbook,
    df: pd.DataFrame,
    column_name: FrameworkContentColumns | str,
    current_sheet_name: str,
    ...
    if column_name not in (
        ...
        raise ValueError(
            f'({fct_name}) [{current_sheet_name}] Column "{column_name}" is not supported for URN validation'
        )
    ...
    urn_meta_sheets = get_meta_sheets_names_from_type(
        wb, MetaTypes.URN_PREFIX, fct_name
    )
    ...
    for meta_sheet_name in urn_meta_sheets:
        content_sheet = get_corresponding_type_sheet_names(
            [meta_sheet_name], SheetTypes.CONTENT
        )[0]
        if content_sheet not in wb.sheetnames:
            raise ValueError(
                f'({fct_name}) [{current_sheet_name}] URN_PREFIX content sheet "{content_sheet}" not found'
            )
        ...
        content_df = content_df.dropna(
            subset=[
                URNPrefixContentColumns.PREFIX_ID.value,
                URNPrefixContentColumns.PREFIX_VALUE.value,
            ],
            how="all",
        )
        content_df = content_df[
            (
                content_df[URNPrefixContentColumns.PREFIX_ID.value]
                .astype(str)
                .str.strip()
                != ""
            )
            | (
                content_df[URNPrefixContentColumns.PREFIX_VALUE.value]
                .astype(str)
                .str.strip()
                != ""
            )
        ...
        for _, row in content_df.iterrows():
            prefix_id = str(
                row.get(URNPrefixContentColumns.PREFIX_ID.value, "")
            ).strip()
            prefix_value = str(
                row.get(URNPrefixContentColumns.PREFIX_VALUE.value, "")
            ).strip()
            if not prefix_id:
                continue
    ...
    ref_ctrl_meta = get_meta_sheets_names_from_type(
        wb, MetaTypes.REFERENCE_CONTROLS, fct_name
    )
    ...
    for meta_sheet_name in urn_meta_sheets:
        urn_content_sheet = get_corresponding_type_sheet_names(
            [meta_sheet_name], SheetTypes.CONTENT
        )[0]
        if urn_content_sheet not in wb.sheetnames:
            raise ValueError(
                f'({fct_name}) [{current_sheet_name}] URN_PREFIX content sheet "{urn_content_sheet}" not found'
            )
        ...
        if threats_meta:
            internal_threats, external_threats, internal_threat_sheets = (
                _URN_prefix_classify_prefix_usage(
                    wb,
                    df_urn,
                    threats_meta,
                    MetaTypes.THREATS,
        ...
        if ref_ctrl_meta:
            internal_ref_ctrl, external_ref_ctrl, internal_ref_ctrl_sheets = (
                _URN_prefix_classify_prefix_usage(
                    wb,
                    df_urn,
                    ref_ctrl_meta,
    ...
    if column_name == FrameworkContentColumns.THREATS.value:
        allowed_prefix_ids = {
            pid
            for pid, (pval, _) in urn_prefix_map.items()
            if pval in all_internal_threats or pval in all_external_threats
        }
        forbidden_prefix_ids = {
            pid
            for pid, (pval, _) in urn_prefix_map.items()
            if pval in all_internal_ref_ctrl or pval in all_external_ref_ctrl
        }
    ...
    for i, prefix_val in enumerate(all_internal_threats):
        ...
        if i < len(all_internal_threat_sheets):
            ...
            content_sheet_name = get_corresponding_type_sheet_names(
                [meta_sheet_name], SheetTypes.CONTENT
            )[0]
    ...
    if column_name == FrameworkContentColumns.THREATS.value:
        internal_prefix_ids = {
            pid
            for pid, (pval, _) in urn_prefix_map.items()
            if pval in all_internal_threats
        }
    else:  # reference_controls
        internal_prefix_ids = {
            pid
            for pid, (pval, _) in urn_prefix_map.items()
            if pval in all_internal_ref_ctrl
        }
    ...
    for prefix_id in internal_prefix_ids:
        ...
        ref_id_column = (
            ThreatsContentColumns.REF_ID.value
            if column_name == FrameworkContentColumns.THREATS.value
            else ReferenceControlsContentColumns.REF_ID.value
        )
        ...
        if verification_errors:
            ...
            for idx, i, urn_id, prefix_id in verification_errors:
                msgs.append(
                    f'   - Row #{idx + 2} (element #{i}) -> ref_id "{urn_id}" with prefix "{prefix_id}" ({prefix_id}:{urn_id})'
                )

            msgs.append(
                f'> 💡 Tip: These IDs must exist in the sheet "{content_sheet}" in column "ref_id".'
            )
            raise ValueError(
                f'({fct_name}) [{current_sheet_name}] Invalid internal references found in column "{column_name}":\n'
                + "\n".join(msgs)
            )
    ...
    if column_name == FrameworkContentColumns.THREATS.value:
        external_prefix_ids = {
            pid
            for pid, (pval, _) in urn_prefix_map.items()
            if pval in all_external_threats
        }
    else:  # reference_controls
        external_prefix_ids = {
            pid
            for pid, (pval, _) in urn_prefix_map.items()
            if pval in all_external_ref_ctrl
        }
    ...
    if external_prefix_ids:
        ...
        if not external_refs:
            msg = (
                f'⚠️  [WARNING] ({fct_name}) [{current_sheet_name}] No YAML files provided as external references to check external references in column "{column_name}" (URN Prefix concerned: '
                + ", ".join(f'"{f}"' for f in external_prefix_ids)
                + ")\n"
        ...
        # Else if there are external references in the Excel file, and YAML file(s) containing external references is(are) given as argument
            ...
            yaml_references_from_files = get_yaml_section_from_files(
                external_refs, yaml_section_type, current_sheet_name, verbose, ctx
            )
            ...
            if not yaml_references_from_files:
                msg = (
                    f'⚠️  [WARNING] ({fct_name}) [{current_sheet_name}] None of the YAML files provided as external references contain a "{yaml_section_type.value}" section (URN prefixes requiring verification of external references: '
                    + ", ".join(f'"{f}"' for f in external_prefix_ids)
                    + ")\n"
                    f'> ❌ Unable to check for external "{yaml_section_type.value}" !\n'
    ...
    if yaml_references_from_files:
        for prefix_id in external_prefix_ids:
            ...
            yaml_ref_ids_map = base_urn_with_ref_ids.get(
                prefix_value, {}
            )  # mapping ref_id -> name | { ref_id1: name1, ... }
            valid_ref_ids = set(
                yaml_ref_ids_map.keys()
            ...
            if verification_errors:
                ...
                msgs.append(
                    f'> 💡 Tip: These Ref. IDs must exist in the YAML file "{yaml_filename}" provided as external references (in the "{yaml_section_type.value}" section of the YAML file).'
                )
                raise ValueError(
                    f'({fct_name}) [{current_sheet_name}] Invalid external references found in column "{column_name}":\n'
                    + "\n".join(msgs)
                )
    ...
    if not external_prefix_ids or (
        external_prefix_ids
        and yaml_external_references_retrieved
        and not not_checked_prefix_id
    ):
        if verbose:
...
def _answers_validate_question_choices(df: pd.DataFrame, sheet_name: str):
    ...
    for row_idx, row in df.iterrows():
        ...
        if question_type in question_types_requiring_choices and (
            pd.isna(question_choices) or str(question_choices).strip() == ""
        ):
    ...
    if problematic_rows:
        raise ValueError(
            ...
            + "\n   - ".join(
                f'Row #{row}: {AnswersContentColumns.QUESTION_TYPE.value} "{question_type}"'
                for row, question_type in problematic_rows
            )
...
def _answers_validate_select_implementation_groups(
    wb: Workbook,
    df: pd.DataFrame,
    sheet_name: str,
    split_regex: str | CommonSeparatorRegex = None,
    ...
    split_pattern = (
        split_regex.value
        if isinstance(split_regex, CommonSeparatorRegex)
        else split_regex or CommonSeparatorRegex.LF.value
    )
    selected_values = []
    for value in non_empty_values:
        selected_values.extend(
            item.strip() for item in re.split(split_pattern, value) if item.strip()
        )
    ...
    implementation_groups_meta_sheets = get_meta_sheets_names_from_type(
        wb, MetaTypes.IMPLEMENTATION_GROUPS, fct_name
    )
    implementation_groups_content_sheets = get_corresponding_type_sheet_names(
        implementation_groups_meta_sheets, SheetTypes.CONTENT
    )
    ...
    for content_sheet in implementation_groups_content_sheets:
        if content_sheet not in wb.sheetnames:
            raise ValueError(
                f'({fct_name}) [{sheet_name}] Missing referenced content sheet "{content_sheet}"'
            )

        content_df = pd.DataFrame(wb[content_sheet].values)
        if content_df.empty:
            raise ValueError(
                f'({fct_name}) [{sheet_name}] Referenced sheet "{content_sheet}" is empty'
            )
        ...
        if ref_column not in content_df.columns:
            raise ValueError(
                f'({fct_name}) [{sheet_name}] Referenced sheet "{content_sheet}" does not contain required column "{ref_column}"'
            )

        valid_implementation_groups.update(
            content_df[ref_column].dropna().astype(str).map(str.strip)
        )

    # Check selected implementation group IDs against the collected Ref. IDs
    validate_allowed_column_values(
        df,
        AnswersContentColumns.SELECT_IMPLEMENTATION_GROUPS,
        tuple(sorted(valid_implementation_groups)),
        sheet_name,
        fct_name,
        split_regex=split_pattern,
    )


# Get the "threats" or "reference_controls" section from a YAML Framework file passed as argument
def get_yaml_section_from_files(
    yaml_files: List[str],
    section_type: YAMLSectionTypes,
    current_sheet_name: str,
    verbose: bool = False,
    ctx: ConsoleContext = None,
) -> Dict:
    ...
    for file in yaml_files:
        ...
        try:
            ...
            raise FileNotFoundError(
                f'({fct_name}) [{current_sheet_name}] YAML file not found: "{file}"'
            )
        except yaml.YAMLError as e:
            raise ValueError(
                f'({fct_name}) [{current_sheet_name}] Error parsing YAML file "{file}": {e}'
            )
        ...
        for obj_key, obj_value in data.get("objects", {}).items():
            ...
            if (
                section_type == YAMLSectionTypes.REFERENCE_CONTROLS
                and obj_key == "reference_controls"
...
def validate_min_max_columns(
    df: pd.DataFrame,
    min_column: ContentColumn | str,
    max_column: ContentColumn | str,
    sheet_name: str,
    context: str,
    min_column_constraints: ContentColumnConstraints | None = None,
    max_column_constraints: ContentColumnConstraints | None = None,
):
    ...
    validate_columns_presence_together(
        df, [min_column, max_column], sheet_name, context
    )
    ...
    min_column_min = (
        min_column_constraints.min_value
        if min_column_constraints and min_column_constraints.min_value is not None
        else 0
    )
    max_column_min = (
        max_column_constraints.min_value
        if max_column_constraints and max_column_constraints.min_value is not None
        else 0
    )

    validate_integer_value(
        df, sheet_name, min_column, context, value_name=min_column, min=min_column_min
    )
    validate_integer_value(
        df, sheet_name, max_column, context, value_name=max_column, min=max_column_min
    )
    ...
    for index, row in df.iterrows():
        ...
        if min_is_empty or max_is_empty:
            incomplete_rows.append(
                f'Row #{excel_row}: "{min_column if min_is_empty else max_column}" is missing'
            )
            continue

        if int(min_value) > int(max_value):
            invalid_ranges.append(
                f'Row #{excel_row}: "{min_column}" ({min_value}) > "{max_column}" ({max_value})'
            )
...
def _framework_validate_depth_consistency(df: pd.DataFrame, sheet_name: str):
    ...
    validate_integer_value(
        ...
        FrameworkContentColumns.DEPTH,
        fct_name,
        value_name=FrameworkContentColumns.DEPTH,
        positive_only=True,
    )

    depth_values = (
        df[FrameworkContentColumns.DEPTH.value].dropna().astype(str).map(str.strip)
    )
    ...
    if first_depth != 1:
        errors.append(
            f'Row #{first_row}: The first "{FrameworkContentColumns.DEPTH.value}" value must be 1. Found "{first_depth}" instead.'
        )

    for (previous_row, previous_depth), (current_row, current_depth) in zip(
        depths, depths[1:]
    ):
...
def _framework_validate_scores_definition_column(
    wb: Workbook, df: pd.DataFrame, sheet_name: str
):
    ...
    meta_sheet_name = get_corresponding_type_sheet_names([sheet_name], SheetTypes.META)[
        0
    ]

    if meta_sheet_name not in wb.sheetnames:
        raise ValueError(
            f'({fct_name}) [{sheet_name}] Missing associated meta sheet "{meta_sheet_name}".'
        )

    meta_df = pd.DataFrame(wb[meta_sheet_name].values)
    default_scores_definition = get_meta_value(
        meta_df, FrameworkMetaKeys.SCORES_DEFINITION, meta_sheet_name, context=fct_name
    )
    ...
    if redundant_values:
        details = "\n   - ".join(
            f'Row #{row}: "{value}"' for row, value in redundant_values
        )
        errors.append(
    ...
    if missing_sheets:
        details = "\n   - ".join(
            f'Row #{row}: "{value}" points to missing sheet "{meta_sheet}"'
            for row, value, meta_sheet in missing_sheets
        )
        errors.append(
...
def validate_framework_content(
    ...
    validate_optional_columns_content_sheet(
        df, sheet_name, schema.optional_columns, fct_name, verbose, ctx
    )
    ...
    question_answer_column_names = [
        FrameworkContentColumns.QUESTIONS,
        FrameworkContentColumns.ANSWER,
    ]
    validate_columns_presence_together(
        df, question_answer_column_names, sheet_name, fct_name
    )

    # Check uniqueness of some column values
    validate_unique_column_values(
        df, [FrameworkContentColumns.REF_ID], sheet_name, fct_name, ctx=ctx
    )
    ...
    if FrameworkContentColumns.ASSESSABLE.value not in df.columns:
        raise ValueError(
            f'[{fct_name}] [{sheet_name}] Missing required column "{FrameworkContentColumns.ASSESSABLE.value}"'
        )

    # Check "assessable" values
    validate_allowed_column_values(
        df,
        FrameworkContentColumns.ASSESSABLE,
        assessable_constraints.allowed_values,
        sheet_name,
        fct_name,
        ctx=ctx,
    )
    ...
    for column in [
        FrameworkContentColumns.IMPLEMENTATION_GROUPS.value,
        FrameworkContentColumns.ANSWER.value,
    ]:
        if column in df.columns:
            non_empty_values = df[column].dropna().astype(str).map(str.strip)
            if not non_empty_values[non_empty_values != ""].empty:
                _framework_validate_column_against_reference_sheet(
                    wb, df, column, sheet_name, verbose, ctx
                )
    ...
    for column in [
        ...
        if column in df.columns:
            non_empty_values = df[column].dropna().astype(str).map(str.strip)
            if not non_empty_values[non_empty_values != ""].empty:
                _framework_validate_framework_column_urns(
                    wb, df, column, sheet_name, external_refs, verbose, ctx
                )
    ...
    validate_allowed_column_values(
        df,
        FrameworkContentColumns.IMPORTANCE,
        importance_constraints.allowed_values,
        sheet_name,
        fct_name,
        ctx=ctx,
    )
    ...
    validate_allowed_column_values(
        df,
        FrameworkContentColumns.DEPENDS_ON,
        depends_on_constraints.allowed_values,
        sheet_name,
        fct_name,
        ctx=ctx,
        split_regex=depends_on_constraints.split_regex,
    )
    ...
    validate_allowed_column_values(
        df,
        FrameworkContentColumns.CONDITION,
        condition_constraints.allowed_values,
        sheet_name,
        fct_name,
        ctx=ctx,
        split_regex=condition_constraints.split_regex,
    )
    ...
    validate_integer_value(
        ...
        FrameworkContentColumns.WEIGHT,
        fct_name,
        value_name=FrameworkContentColumns.WEIGHT,
        min=weight_constraints.min_value,
        max=weight_constraints.max_value,
    )

    # Validate "min_score" and "max_score" values if columns are present
    validate_min_max_columns(
        df,
        FrameworkContentColumns.MIN_SCORE,
        FrameworkContentColumns.MAX_SCORE,
        sheet_name,
        fct_name,
        min_score_constraints,
        max_score_constraints,
    )
...
def validate_threats_content(
    ...
    # Check uniqueness of some column values
    validate_unique_column_values(
        df, [ThreatsContentColumns.REF_ID], sheet_name, fct_name, ctx=ctx
    )
...
# [CONTENT] TTP Catalog
def validate_ttp_catalog_content(
    df: pd.DataFrame, sheet_name: str, verbose: bool = False, ctx: ConsoleContext = None
):

    fct_name = get_current_fct_name()

    content_type = MetaTypes.TTP_CATALOG
    schema = CONTENT_SHEET_SCHEMAS[content_type]
    ...
    validate_unique_column_values(
        df, [TTPCatalogContentColumns.REF_ID], sheet_name, fct_name, ctx=ctx
    )
...
# [CONTENT] TTP Groups
def validate_ttp_groups_content(
    df: pd.DataFrame, sheet_name: str, verbose: bool = False, ctx: ConsoleContext = None
):

    fct_name = get_current_fct_name()

    content_type = MetaTypes.TTP_GROUPS
    schema = CONTENT_SHEET_SCHEMAS[content_type]
    ...
    validate_unique_column_values(
        df, [TTPGroupsContentColumns.REF_ID], sheet_name, fct_name, ctx=ctx
    )
...
# [CONTENT] Tactics
def validate_tactics_content(
    df: pd.DataFrame, sheet_name: str, verbose: bool = False, ctx: ConsoleContext = None
):

    fct_name = get_current_fct_name()

    content_type = MetaTypes.TACTICS
    schema = CONTENT_SHEET_SCHEMAS[content_type]
    ...
    validate_unique_column_values(
        df, [TacticsContentColumns.REF_ID], sheet_name, fct_name, ctx=ctx
    )
...
# [CONTENT] Techniques
def validate_techniques_content(
    wb: Workbook,
    df: pd.DataFrame,
    sheet_name: str,
    verbose: bool = False,
    ...
    content_type = MetaTypes.TECHNIQUES
    schema = CONTENT_SHEET_SCHEMAS[content_type]
    ...
    validate_unique_column_values(
        df, [TechniquesContentColumns.REF_ID], sheet_name, fct_name, ctx=ctx
    )

    validate_technique_parents(df, sheet_name, fct_name, ctx)
    validate_technique_tactic_refs(wb, df, sheet_name, fct_name, ctx)
...
# A parent_ref_id must name another technique of the same sheet, and never itself
def validate_technique_parents(
    df: pd.DataFrame, sheet_name: str, context: str, ctx: ConsoleContext = None
):

    parent_column = TechniquesContentColumns.PARENT_REF_ID.value
    ref_id_column = TechniquesContentColumns.REF_ID.value
    if parent_column not in df.columns or ref_id_column not in df.columns:
        return

    known_ref_ids = {str(value).strip() for value in df[ref_id_column].dropna()}

    for position, (ref_id, parent_ref_id) in enumerate(
        zip(df[ref_id_column], df[parent_column]), start=2
    ):
        if pd.isna(parent_ref_id) or not str(parent_ref_id).strip():
            continue
        parent_ref_id = str(parent_ref_id).strip()
        ref_id = str(ref_id).strip()

        if parent_ref_id == ref_id:
            raise ValueError(
                f'({context}) [{sheet_name}] Row #{position}: Technique "{ref_id}" is its own parent'
                f'\n> 💡 Tip: Leave "{parent_column}" empty for a top-level technique.'
            )

        if parent_ref_id not in known_ref_ids:
            raise ValueError(
                f'({context}) [{sheet_name}] Row #{position}: Unknown parent "{parent_ref_id}" for technique "{ref_id}"'
                f'\n> 💡 Tip: "{parent_column}" must match the "{ref_id_column}" of another technique in this sheet.'
            )


# Every tactic_ref_ids entry must exist in the tactics content sheet
def validate_technique_tactic_refs(
    wb: Workbook,
    df: pd.DataFrame,
    sheet_name: str,
    context: str,
    ...
    tactics_column = TechniquesContentColumns.TACTIC_REF_IDS.value
    if tactics_column not in df.columns:
        return

    known_tactics = set()
    for candidate in wb.sheetnames:
        if not candidate.endswith(SheetTypes.CONTENT.value):
            continue
        meta_name = re.sub(r"_content$", SheetTypes.META.value, candidate)
        if meta_name not in wb.sheetnames:
            continue
        meta_df = pd.DataFrame(wb[meta_name].values)
        if (
            get_meta_value(meta_df, MandatoryMetaKeys.TYPE, meta_name, context=context)
            != MetaTypes.TACTICS.value
        ):
            continue
        tactics_df = pd.DataFrame(wb[candidate].values)
        tactics_df.columns = tactics_df.iloc[0]
        tactics_df = tactics_df.drop(index=0).reset_index(drop=True)
        ref_id_column = TacticsContentColumns.REF_ID.value
        if ref_id_column in tactics_df.columns:
            known_tactics.update(
                str(value).strip() for value in tactics_df[ref_id_column].dropna()
            )

    if not known_tactics:
        return

    for position, value in enumerate(df[tactics_column], start=2):
        if pd.isna(value) or not str(value).strip():
            continue
        for ref_id in re.split(r"[\s,]+", str(value).strip()):
            if ref_id and ref_id not in known_tactics:
                raise ValueError(
                    f'({context}) [{sheet_name}] Row #{position}: Unknown tactic "{ref_id}"'
                    f'\n> 💡 Tip: Every entry of "{tactics_column}" must match a "{TacticsContentColumns.REF_ID.value}" of the tactics content sheet.'
                )


# [CONTENT] Reference Controls {OK}²
def validate_reference_controls_content(
    df: pd.DataFrame, sheet_name: str, verbose: bool = False, ctx: ConsoleContext = None
):
    ...
    csf_function_constraints = column_constraints[
        ReferenceControlsContentColumns.CSF_FUNCTION
    ]
    ...
    # Check uniqueness of some column values
    validate_unique_column_values(
        df, [ReferenceControlsContentColumns.REF_ID], sheet_name, fct_name, ctx=ctx
    )
    ...
    validate_allowed_column_values(
        df,
        ReferenceControlsContentColumns.CATEGORY,
        category_constraints.allowed_values,
        sheet_name,
        fct_name,
        ctx=ctx,
    )
    validate_allowed_column_values(
        df,
        ReferenceControlsContentColumns.CSF_FUNCTION,
        csf_function_constraints.allowed_values,
        sheet_name,
        fct_name,
        ctx=ctx,
    )
...
# [CONTENT] Risk Matrix²
def validate_risk_matrix_content(
    df: pd.DataFrame, sheet_name: str, verbose: bool = False, ctx: ConsoleContext = None
):
    ...
    validate_allowed_column_values(
        df,
        RiskMatrixContentColumns.TYPE,
        type_constraints.allowed_values,
        sheet_name,
        fct_name,
        ctx=ctx,
    )
...
def validate_implementation_groups_content(
    wb: Workbook,
    df: pd.DataFrame,
    sheet_name: str,
    verbose: bool = False,
    ...
    default_selected_constraints = column_constraints[
        ImplementationGroupsContentColumns.DEFAULT_SELECTED
    ]
    ...
    # Check uniqueness of some column values
    validate_unique_column_values(
        df, [ImplementationGroupsContentColumns.REF_ID], sheet_name, fct_name, ctx=ctx
    )
    ...
    validate_allowed_column_values(
        df,
        ImplementationGroupsContentColumns.DEFAULT_SELECTED,
        default_selected_constraints.allowed_values,
        sheet_name,
        fct_name,
        ctx=ctx,
    )
    ...
    frameworks_with_imp_grp = check_content_sheet_usage_in_frameworks(
        wb,
        sheet_name,
        FrameworkMetaKeys.IMPLEMENTATION_GROUPS_DEFINITION,
        fct_name,
    ...
    frameworks_with_imp_grp = get_corresponding_type_sheet_names(
        frameworks_with_imp_grp, SheetTypes.CONTENT
    )

    # Check if every implementation groups are actually used in "framework" sheets
    if frameworks_with_imp_grp:
        _implementation_groups_check_unused_default_ids_in_frameworks(
            wb, df, frameworks_with_imp_grp, sheet_name
        )
        check_unused_ids_in_frameworks(
            wb,
            df,
            ImplementationGroupsContentColumns.REF_ID,
            FrameworkContentColumns.IMPLEMENTATION_GROUPS,
            frameworks_with_imp_grp,
            sheet_name,
            fct_name,
            ctx,
            verbose,
...
def validate_requirement_mapping_set_content(
    wb: Workbook,
    df: pd.DataFrame,
    sheet_name: str,
    verbose: bool = False,
    ...
    relationship_constraints = column_constraints[
        RequirementMappingSetContentColumns.RELATIONSHIP
    ]
    rationale_constraints = column_constraints[
        RequirementMappingSetContentColumns.RATIONALE
    ]
    ...
    validate_allowed_column_values(
        df,
        RequirementMappingSetContentColumns.RELATIONSHIP,
        relationship_constraints.allowed_values,
        sheet_name,
        fct_name,
        ctx=ctx,
    )
    validate_allowed_column_values(
        df,
        RequirementMappingSetContentColumns.RATIONALE,
        rationale_constraints.allowed_values,
        sheet_name,
        fct_name,
        ctx=ctx,
    )
    ...
    _req_map_set_validate_mapping_node_ids_against_sheets(
        wb, df, sheet_name, fct_name, ctx, verbose
    )
...
def validate_scores_content(
    ...
    validate_integer_value(
        df,
        sheet_name,
        ScoresContentColumns.SCORE,
        fct_name,
        value_name=ScoresContentColumns.SCORE,
        min=score_constraints.min_value,
        max=score_constraints.max_value,
    )

    # Check uniqueness of some column values
    validate_unique_column_values(
        df, [ScoresContentColumns.SCORE], sheet_name, fct_name, ctx=ctx
    )
    ...
    check_content_sheet_usage_in_frameworks(
        wb, sheet_name, FrameworkMetaKeys.SCORES_DEFINITION, fct_name, ctx
    )
...
def validate_answers_content(
    ...
    question_choices_constraints = column_constraints[
        AnswersContentColumns.QUESTION_CHOICES
    ]
    description_constraints = column_constraints[AnswersContentColumns.DESCRIPTION]
    select_implementation_groups_constraints = column_constraints[
        AnswersContentColumns.SELECT_IMPLEMENTATION_GROUPS
    ]
    ...
    # Check uniqueness of some column values
    validate_unique_column_values(
        df, [AnswersContentColumns.ID], sheet_name, fct_name, ctx=ctx
    )
    ...
    validate_allowed_column_values(
        df,
        AnswersContentColumns.QUESTION_TYPE,
        question_type_constraints.allowed_values,
        sheet_name,
        fct_name,
        ctx=ctx,
    )
    ...
    validate_allowed_column_values(
        df,
        AnswersContentColumns.COMPUTE_RESULT,
        compute_result_contraints.allowed_values,
        sheet_name,
        fct_name,
        ctx=ctx,
        split_regex=compute_result_contraints.split_regex,
    )
    ...
    validate_allowed_column_values(
        df,
        AnswersContentColumns.COLOR,
        color_constraints.allowed_values,
        sheet_name,
        fct_name,
        ctx=ctx,
        split_regex=color_constraints.split_regex,
    )
    ...
    validate_integer_value(
        ...
        AnswersContentColumns.ADD_SCORE,
        fct_name,
        value_name=AnswersContentColumns.ADD_SCORE,
        min=add_score_constraints.min_value,
        split_regex=add_score_constraints.split_regex,
    )

    # Check if values in "select_implementation_groups" exist in an implementation groups sheet
    _answers_validate_select_implementation_groups(
        wb, df, sheet_name, select_implementation_groups_constraints.split_regex
    )
    ...
    frameworks_with_answers = check_content_sheet_usage_in_frameworks(
        wb, sheet_name, FrameworkMetaKeys.ANSWERS_DEFINITION, fct_name, ctx
    )
    frameworks_with_answers = get_corresponding_type_sheet_names(
        frameworks_with_answers, SheetTypes.CONTENT
    )
    ...
    validate_cell_line_count_alignment(
        ...
        fct_name,
        cmp_can_be_empty=True,
        ref_line_break_indicator=question_choices_constraints.line_break_indicator,
        cmp_line_break_indicator=description_constraints.line_break_indicator,
    )
...
def validate_urn_prefix_content(
    wb: Workbook,
    df: pd.DataFrame,
    sheet_name: str,
    verbose: bool = False,
    ...
    validate_unique_column_values(
        df,
        [URNPrefixContentColumns.PREFIX_ID, URNPrefixContentColumns.PREFIX_VALUE],
        sheet_name,
        fct_name,
        ctx=ctx,
    )
    ...
    _URN_prefix_validate_prefix_values_and_dependencies(
        wb, df, sheet_name, ctx, verbose
    )
...
def dispatch_meta_validation(
    ...
    type_value = get_meta_value(
        df, MandatoryMetaKeys.TYPE, sheet_name, required=True, context=fct_name
    )
    ...
    if type_value == MetaTypes.LIBRARY.value:
    ...
    elif type_value == MetaTypes.TTP_CATALOG.value:
        validate_ttp_catalog_meta(wb, df, sheet_name, verbose, ctx)
    elif type_value == MetaTypes.TTP_GROUPS.value:
        validate_ttp_groups_meta(wb, df, sheet_name, verbose, ctx)
    elif type_value == MetaTypes.TACTICS.value:
        validate_tactics_meta(df, sheet_name, verbose, ctx)
    elif type_value == MetaTypes.TECHNIQUES.value:
        validate_techniques_meta(df, sheet_name, verbose, ctx)
        ...
        raise ValueError(
            f'({fct_name}) [{sheet_name}] Unknown meta type "{type_value}"'
        )


def dispatch_content_validation(
    wb: Workbook,
    df: pd.DataFrame,
    sheet_name: str,
    corresponding_meta_type: str,
    external_refs: List[str] = None,
    verbose: bool = False,
    ctx: ConsoleContext = None,
):
    ...
    if corresponding_meta_type == MetaTypes.FRAMEWORK.value:
    ...
    elif corresponding_meta_type == MetaTypes.TTP_CATALOG.value:
        validate_ttp_catalog_content(df, sheet_name, verbose, ctx)
    elif corresponding_meta_type == MetaTypes.TTP_GROUPS.value:
        validate_ttp_groups_content(df, sheet_name, verbose, ctx)
    elif corresponding_meta_type == MetaTypes.TACTICS.value:
        validate_tactics_content(df, sheet_name, verbose, ctx)
    elif corresponding_meta_type == MetaTypes.TECHNIQUES.value:
        validate_techniques_content(wb, df, sheet_name, verbose, ctx)
        ...
        raise ValueError(
            f'({fct_name}) [{sheet_name}] Cannot determine validation for content of type "{corresponding_meta_type}"'
        )
...
def validate_excel_structure(
    ...
    if external_refs:
        check_file_validity(
            external_refs, "YAML", ValidFileTypes.YAML.value, "External Reference"
        )
    ...
    for sheet_name, df in meta_sheets.items():
        ...
        if (
            sheet_name != MandatorySheets.LIBRARY_META.value
            and expected_content_sheet not in content_sheets
        ):
            raise ValueError(
                f"({fct_name}) [{sheet_name}] No corresponding content sheet found for this meta"
        ...
        meta_types[base_name] = get_meta_value(
            df, MandatoryMetaKeys.TYPE, sheet_name, required=True, context=fct_name
        )
    ...
    for sheet_name, df in content_sheets.items():
        ...
        if base_name not in meta_types:
            raise ValueError(
                f"({fct_name}) [{sheet_name}] No corresponding meta sheet found for this content"
    ...
    for sheet_name, df in content_sheets.items():
        base_name = re.sub(r"_content$", "", sheet_name)
        dispatch_content_validation(
            wb, df, sheet_name, meta_types[base_name], external_refs, verbose, ctx
        )
    ...
    if ctx.count_all_warnings() > 0:
        print(
            f'✅⚠️  [SUCCESS] Excel structure validation ended with warnings for "{file_name}"'
        )
        print(
            f'📜 [SUMMARY] ⚠️  Total [WARNING] for "{file_name}": {ctx.count_all_warnings()}'
        )
    else:
    ...
    if verbose and ctx.count_all_verbose() > 0:
        print(
            f'📜 [SUMMARY] 💬 Total [Verbose Messages] for "{file_name}": {ctx.count_all_verbose()}'
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Validate Excel file structure (v2 format)",
        formatter_class=argparse.RawTextHelpFormatter,
    )
    parser.add_argument("file_input", help="Path to Excel file to validate.")
...
def bulk_check(
    args: argparse.Namespace, external_refs: List[str] = None, enable_ctx: bool = False
) -> tuple[Dict[str, ConsoleContext], List[str]]:
    ...
    if warning_files:
        print(
            f"⚠️  The following file{'s' if len(error_files) > 1 else ''} encountered at least 1 warning:"
        )
