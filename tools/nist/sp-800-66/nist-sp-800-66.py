...
for tab in dataframe:
    ...
    if title == "NIST SP 800-66":
        ...
        for row in tab:
            line += 1
            if line > 1:
                (security_rule_id, security_rule, std_id, std, key_activity, description, sample_questions) = (r.value for r in row)
                if security_rule_id:
...
ws.append(['framework_urn', f'urn:{packager.lower()}:risk:framework:nist-sp-800-66-rev2'])
