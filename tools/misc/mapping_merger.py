...
# Merge A -> B -> C
merged_df = df1.merge(
    df2, left_on="target_node_id", right_on="source_node_id", how="left"
)
...
if not unmatched.empty:
    ...
    print(
        "⚠️  [WARNING] The following target_node_id values from file 1 (A to B) had no match in file 2 (B to C):"
    )
...
try:
    ...
    print(
        f'❌ [ERROR] Could not write output Excel file "{output_path}".\nDetails: {e}'
    )
    print(
        "💡 Tip: Make sure the file is not open in Excel or locked by another process."
    )
    sys.exit(1)
...
if not unmatched.empty:
    print(
        f'⚠️  [WARNING] {len(unmatched)} mapping{"s" if len(unmatched) > 1 else ""} could not be done and were written to the "warning" sheet.'
    )
