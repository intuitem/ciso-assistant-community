...
def get_requirement_mappings(data: dict) -> tuple[list[dict], int]:
    ...
    if "requirement_mapping_set" in objects:
        ...
        if "requirement_mappings" not in rms:
            print(
                "❌ [ERROR] Missing 'requirement_mappings' in 'requirement_mapping_set'."
            )
            sys.exit(1)
    ...
    # Version 2
        ...
        while True:
            choice = input(
                "ℹ️  Version 2 detected. Choose which mapping to export:\n"
        ...
        if choice == "3":
            if len(sets) < 2:
                print(
                    "❌ [ERROR] Cannot export both mappings: only one mapping set found."
                )
                sys.exit(1)
            for idx, s in enumerate(sets[:2]):
                if "requirement_mappings" not in s:
                    print(
                        f"❌ [ERROR] Missing 'requirement_mappings' in mapping set {idx}."
                    )
                    sys.exit(1)
            return sets, 3
        else:
            ...
            if idx >= len(sets):
                print(
                    f"❌ [ERROR] Mapping index {idx} not found in 'requirement_mapping_sets'."
                )
                sys.exit(1)
            if "requirement_mappings" not in sets[idx]:
                print(
                    f"❌ [ERROR] Missing 'requirement_mappings' in selected mapping set."
                )
                sys.exit(1)
        ...
        print(
            "❌ [ERROR] Neither 'requirement_mapping_set' nor 'requirement_mapping_sets' found."
        )
        sys.exit(1)


def convert_yaml_to_excel(
    yaml_file: str, mappings_data, excel_file: str, mapping_index: int
):
    warnings = []
        ...
        for i, item in enumerate(mappings):
            ...
            if missing_fields:
                ...
                warning_entry = {
                    ...
                    "YAML line number": line_number + 1
                ...
                print(
                    f'⚠️  [WARNING] Missing field(s) in sheet "{mapping_name}" (Excel row {excel_row}, YAML line {warning_entry["YAML line number"]}) → {warning_entry["Missing fields"]}'
                )
                ...
                for field in missing_fields:
                    ...
                    if col_letter:
                        missing_cell_positions[mapping_name].append(
                            (col_letter, excel_row)
                        )
    ...
    if mapping_index == 3:
        sheet_data = [
            (
                "mappings",
                process_mapping(
                    mappings_data[0].get("requirement_mappings", []), "mappings"
                ),
            ),
            (
                "mappings_revert",
                process_mapping(
                    mappings_data[1].get("requirement_mappings", []), "mappings_revert"
                ),
            ),
        ]
    else:
    ...
    try:
        with pd.ExcelWriter(excel_file, engine="openpyxl") as writer:
            ...
            info_sheet["A3"] = (
                "Please note that this Excel file doesn't replace the one generated by prepare_mapping.py."
            )
        ...
        print(f"→  Reason: {e}")
        print(
            "💡 Tip: Make sure the file isn't already open in Excel or another program."
        )
        sys.exit(1)
    ...
    if warnings:
        print(
            f'⚠️  {len(warnings)} warnings found. See the "warning" sheet in the output file.'
        )
