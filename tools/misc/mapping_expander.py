    ...
    try:
        with open(path, "r", encoding="utf-8") as f:
            ...
            if framework_exception:
                print(
                    f'ℹ️  Exceptions list for "{framework_exception}" loaded from JSON file "{os.path.basename(path)}"'
                )
    ...
    for index, row in df_source.iterrows():
        ...
        if framework_exception in exception_map:
            ...
            if search_prefix != prefix:
                print(
                    f'ℹ️  Exception applied for framework "{framework_exception}": "{prefix}" mapped to "{search_prefix}" ({fixed_column}: "{fixed_value}") [row #{index + 2}]'
                )
        ...
        matches = [
            node_id for node_id in node_id_list if node_id.startswith(search_prefix)
        ]
...
def run_expansion(reverse=False):
    ...
    df_reference = pd.read_excel(
        reference_file, sheet_name=reference_sheet_name, dtype=str
    )
    node_id_list = df_reference[reference_column_name].dropna().tolist()

    df_result, warnings_list, exceptions_list = expand_mappings(
        df_source, node_id_list, exception_map, reverse
    )
    ...
    rows_info = [
        {
            "text": f"Mapping Expander v{SCRIPT_VERSION}",
        ...
        {
            "text": f"Reference sheet: {reference_sheet_name} | Ref Column : {reference_column_name}",
            "font": Font(size=15),
        },
        {
    ...
    for column in ws_mappings.columns:
        for cell in column:
            cell.number_format = (
                numbers.FORMAT_TEXT
            )  # Force cells as text (avoid auto-formatting of IDs)
    ...
    if exceptions_list:
        ...
        ws_exc.append(
            ["source_node_id", "original_target_node_id", "mapped_target_node_id"]
        )
    ...
    print(
        f'➡ Reference sheet: "{reference_sheet_name}", column: "{reference_column_name}"'
    )
    print(f'📁 Output file: "{destination_file}"')
    if warnings_list:
        print(
            f'⚠️  {len(warnings_list)} warnings found. See the "warnings" sheet in the output file.'
        )
    if exceptions_list:
        print(
            f'ℹ️  {len(exceptions_list)} exceptions applied. See the "exceptions" sheet in the output file.'
        )


def main():
    parser = argparse.ArgumentParser(
        description="Expand mapping by target_node_id (default) or source_node_id (with --reverse)."
    )
    parser.add_argument(
        "--reverse",
        action="store_true",
        help="Expand using source_node_id instead of target_node_id",
    )
