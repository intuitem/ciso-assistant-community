...
def validate_file_size(value):
    ...
    if filesize > int(settings.ATTACHMENT_MAX_SIZE_MB)*1000000:
        raise ValidationError(f"The maximum file size that can be uploaded is {settings.ATTACHMENT_MAX_SIZE_MB} MB")
    else:
        return value
...
def validate_file_name(value):
    ...
    """
    allowed_extensions = ['jpg', 'jpeg', 'png', 'docx', 'txt', 'xls', 'xlsx', 'csv', 'pdf']
    ...
    if extension in allowed_extensions:
        ...
        value.name = slugify(get_valid_filename(value.name.replace(extension, ''))) + '.' + extension
        return value
    else:
