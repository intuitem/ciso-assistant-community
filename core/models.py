...
class Analysis(AbstractBaseModel):
    ...
    def quality_check(self) -> dict:
        ...
        for ri in self.riskscenario_set.all().order_by('created_at'):
            ...
            if ri.residual_level > ri.current_level:
                errors_lst.append({"msg": _("{} residual risk level is higher than the current one").format(
                    ri), "obj_type": "riskscenario", "object": ri})
            ...
            if ri.residual_level < ri.current_level or ri.residual_proba < ri.current_proba or ri.residual_impact < ri.current_impact:
                if ri.security_measures.count() == 0:
                    errors_lst.append(
                        {"msg": _("{}: residual risk level has been lowered without any specific measure").format(ri), "obj_type": "riskscenario", "object": ri})
        ...
        for mtg in SecurityMeasure.objects.filter(riskscenario__analysis=self):
            if not mtg.eta:
                warnings_lst.append({"msg": _("{}/{} does not have an ETA").format(mtg.folder,
                    mtg), "obj_type": "securitymeasure", "object": mtg})
            else:
                if date.today() > mtg.eta:
                    errors_lst.append(
                        {"msg": _("{}/{} ETA is in the past now. Consider updating its status or the date").format(mtg.folder, mtg), "obj_type": "securitymeasure", "object": mtg})
            if not mtg.effort:
                warnings_lst.append(
        ...
        for ra in RiskAcceptance.objects.filter(risk_scenarios__analysis=self):
            if not ra.expiry_date:
                warnings_lst.append({"msg": _("{}/{}: Acceptance has no expiry date").format(ra.folder,
                    ra), "obj_type": "securitymeasure", "object": ra})
                continue
            if date.today() > ra.expiry_date:
                errors_lst.append(
                    {"msg": _("{}/{}: Acceptance has expired. Consider updating the status or the date").format(ra.folder, ra), "obj_type": "riskacceptance", "object": ra})
