...
class RiskAcceptanceDetailView(GenericDetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['validate_riskacceptance'] = RoleAssignment.has_permission(self.request.user, 'validate_riskacceptance') and (self.object.folder.id in RoleAssignment.get_accessible_folders(
            Folder.objects.get(content_type=Folder.ContentType.ROOT), self.request.user, Folder.ContentType.DOMAIN)) or (UserGroup.objects.get(name="BI-UG-GVA") in UserGroup.get_user_groups(self.request.user))
        return context
        ...
        if RoleAssignment.has_permission(self.request.user, 'validate_riskacceptance') and (self.object.folder.id in RoleAssignment.get_accessible_folders(Folder.objects.get(content_type=Folder.ContentType.ROOT), self.request.user, Folder.ContentType.DOMAIN)):
            if 'accepted' in request.POST:
                ...
                messages.success(request, _(
                    "Risk acceptance accepted with success"))
            elif 'rejected' in request.POST:
                self.object.set_state('rejected')
                messages.success(request, _(
                    "Risk acceptance rejected with success"))
            elif 'revoked' in request.POST:
                self.object.set_state('revoked')
                messages.success(request, _(
                    "Risk acceptance revoked with success"))
            else:
                messages.error(request, "An error has occured")
        else:
            messages.error(
                request, "Permission denied: you are not validator or you've not this role in this risk acceptance folder")
