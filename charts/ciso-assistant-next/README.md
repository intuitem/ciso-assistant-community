...
restarted by hand (same as `djangoExistingSecretKey` and the SMTP one). When manifests are rendered without
cluster access (`helm template`, ArgoCD, ...), the chart cannot read the generated key back and would
issue a new one on each render : set `existingSecret` or `privateKey` in that case.
