...
export class ModelTableRow extends Element {
	...
	protected _importLoadingSpin: Locator;
	...
	constructor(...args: Element.Args) {
		...
		this._importLoadingSpin = this._self.getByTestId('table-row-import-loading-spin-elem');
	 ...
	 */
	async doLoadLibrary(expect: Expect, existsOk: boolean = false): Promise<void> {
		if (existsOk) {
			const isVisible = await this._importButton.isVisible();
			if (!isVisible) return;
		}
		await this._importButton.click();
		await expect(this._importLoadingSpin).not.toBeVisible();
	}
