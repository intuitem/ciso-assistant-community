import { test, expect, setHttpResponsesListener, TestContent, replaceValues } from '../../utils/test-utils.js';
...
for (const key of testPages) {
    test.describe(`Tests on ${items[key].displayName.toLowerCase()} item`, () => {
        test.beforeAll(async ({}, testInfo) => {
            ...
            existsSync(file_path) ? history = JSON.parse(readFileSync(file_path, 'utf8')) : writeFileSync(file_path, JSON.stringify(history));
        });
