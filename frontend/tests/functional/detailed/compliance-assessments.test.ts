...
test('compliance assessments scoring is working properly', async ({ logedPage, pages, complianceAssessmentsPage, page }) => {
    ...
    IDAM1SliderBoundingBox && await page.getByTestId('range-slider-input').click({position: {x: IDAM1SliderBoundingBox.width * IDAM1Score.ratio, y: IDAM1SliderBoundingBox.height / 2}});
    await expect(page.getByTestId('progress-radial')).toHaveAttribute('aria-valuenow', IDAM1Score.progress);

    await complianceAssessmentsPage.form.saveButton.click();
    await page.waitForURL(complianceAssessmentsPage.url + '/**');
    await expect(IDAM1TreeViewItem.progressRadial).toHaveAttribute('aria-valuenow', IDAM1Score.progress);
    
    // Click on the ID.AM-2 tree view item
    const IDAM2TreeViewItem = await complianceAssessmentsPage.itemDetail.treeViewItem('ID.AM-2', ['ID - Identify', 'ID.AM - Asset Management']);
    ...
    IDAM2SliderBoundingBox && await page.getByTestId('range-slider-input').click({position: {x: IDAM2SliderBoundingBox.width * IDAM2Score.ratio, y: IDAM2SliderBoundingBox.height / 2}});
    await expect(page.getByTestId('progress-radial')).toHaveAttribute('aria-valuenow', IDAM2Score.progress);

    await complianceAssessmentsPage.form.saveButton.click();
    await page.waitForURL(complianceAssessmentsPage.url + '/**');
    await expect(IDAM2TreeViewItem.progressRadial).toHaveAttribute('aria-valuenow', IDAM2Score.progress);

    // Click on the ID.BE-1 tree view item
    const IDBE1TreeViewItem = await complianceAssessmentsPage.itemDetail.treeViewItem('ID.BE-1', ['ID - Identify', 'ID.BE - Business Environment']);
    ...
    IDBE1SliderBoundingBox && await page.getByTestId('range-slider-input').click({position: {x: IDBE1SliderBoundingBox.width * IDBE1Score.ratio, y: IDBE1SliderBoundingBox.height / 2}});
    await expect(page.getByTestId('progress-radial')).toHaveAttribute('aria-valuenow', IDBE1Score.progress);

    await complianceAssessmentsPage.form.saveButton.click();
    await page.waitForURL(complianceAssessmentsPage.url + '/**');
    await expect(IDBE1TreeViewItem.progressRadial).toHaveAttribute('aria-valuenow', IDBE1Score.progress);

    // Click on the PR.AC-1 tree view item
    const PRAC1TreeViewItem = await complianceAssessmentsPage.itemDetail.treeViewItem('PR.AC-1', ['PR - Protect', 'PR.AC - Identity Management, Authentication and Access Control']);
    ...
    PRAC1SliderBoundingBox && await page.getByTestId('range-slider-input').click({position: {x: PRAC1SliderBoundingBox.width * PRAC1Score.ratio, y: PRAC1SliderBoundingBox.height / 2}});
    await expect(page.getByTestId('progress-radial')).toHaveAttribute('aria-valuenow', PRAC1Score.progress);

    await complianceAssessmentsPage.form.saveButton.click();
    await page.waitForURL(complianceAssessmentsPage.url + '/**');
    await expect(PRAC1TreeViewItem.progressRadial).toHaveAttribute('aria-valuenow', PRAC1Score.progress);
