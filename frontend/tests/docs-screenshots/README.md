...
To document a feature you just built, before the fixture covers it:
...
Nothing is seeded and no server is started, so the shots reflect your own dev
data and are **not reproducible**. Both the URL and the `--grep` selection are
mandatory: the capture writes straight into `product-docs/.gitbook/assets/`, and
without them a stray run replaces every committed screenshot with images of
whatever was listening on that port.

Attached shots cannot be regenerated by anyone who lacks that dev state, so
they age the same way the old externally hosted images did. That is an accepted
trade for feature documentation; the fixture exists for the pages that must stay
regenerable.

**Screen them before they ship.** A dev database usually holds real customer and
prospect data — names in list views, the signed-in user's address in the sidebar,
imported spreadsheet content. Published GitBook pages are public.
...
- Sidebar sections render collapsed; expand them in an `act` step if a shot
