	...
	async treeViewItem(value: string, path: string[] = []) {
		if (path.length !== 0) {
			...
			for (let i=0; i<tree.length - 1; i++) {
				if (await this.page.getByTestId('tree-item-content').getByText(tree[i+1]).isHidden()) {
					await this.page.getByTestId('tree-item-content').getByText(tree[i]).click();
				}
			}
		}
		const content = this.page.getByTestId('tree-item-content').filter({hasText: new RegExp(`^${value}\n*.*`)});
		return {
			content: content,
			progressRadial: this.page.getByTestId('tree-item')
