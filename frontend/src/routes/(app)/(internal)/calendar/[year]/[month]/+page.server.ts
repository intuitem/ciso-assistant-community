...
export const load = (async ({ fetch, locals, params }) => {
	...
	const tasksEndpoint = `${BASE_API_URL}/users/${locals.user.id}/tasks/${params.year}-${params.month.padStart(2, '0')}-01/${nextMonth(parseInt(params.month)) === '01' ? parseInt(params.year) + 1 : params.year}-${nextMonth(parseInt(params.month))}-01`;
