<script lang="ts">
	...
	const contract = $derived(data.data);
...
<DetailView {data}>
	...
	{#snippet widgets()}
		{#if page.data?.featureflags?.validation_flows && contract.validation_flows}
			{#key contract.validation_flows}
				<ValidationFlowsSection validationFlows={contract.validation_flows} />
			{/key}
		{/if}
	{/snippet}
</DetailView>
