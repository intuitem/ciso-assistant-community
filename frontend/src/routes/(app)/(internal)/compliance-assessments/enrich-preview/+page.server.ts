...
export const load = (async ({ fetch, url }) => {
	...
	const previewEndpoint = `${BASE_API_URL}/compliance-assessments/${targetId}/enrich_preview/?source_audit_id=${sourceId}`;
	...
	const previewData = await fetch(previewEndpoint).then((res) => {
		if (!res.ok) {
			...
			throw error(500, 'Failed to load enrichment preview');
		}
		return res.json();
	});
	...
	return {
		...
		targetId,
		sourceId,
		title: `Enrich Preview: ${previewData.target_audit.name}`
	};
