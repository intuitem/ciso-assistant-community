	...
	default: async (event) => {
		...
		if (!res.ok) {
			...
			try {
				...
				console.warn('reset-mfa: backend returned a non-JSON error body', err);
			}
			const errorKey = typeof payload.error === 'string' ? payload.error : 'errorResettingMFA';
			setFlash({ type: 'error', message: safeTranslate(errorKey) }, event);
			return fail(res.status, { error: errorKey });
		}

		setFlash({ type: 'success', message: m.mfaResetSuccessfully() }, event);
