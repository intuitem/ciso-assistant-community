	...
	$effect(() => {
		...
		if (
			canPerformAction({
				...
				domain: data.requirementAssessment.folder.id
			}) &&
								...
								<Tabs.Content value="applied_controls">
									...
									<div class="h-full flex flex-col space-y-2 rounded-container p-4">
										<span class="flex flex-row justify-end items-center space-x-2">
											{#if canPerformAction( { user: page.data.user, action: 'add', model: 'appliedcontrol', domain: data.requirementAssessment.folder.id } ) && reference_controls.length > 0}
