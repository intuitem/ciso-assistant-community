	...
	{#each attackPaths as path}
		<div class="text-sm border-l-4 border-primary-500 pl-3 py-2">
			...
			<div class="flex flex-wrap items-center gap-2 font-mono text-xs">
				<span class="px-2 py-1 bg-red-100 text-red-800 dark:bg-red-500/20 dark:text-red-300 rounded font-semibold">
					{safeTranslate(path.risk_origin) || 'Unknown RO'}
				</span>
				<i class="fa-solid fa-arrow-right text-surface-400-600"></i>

				<span class="px-2 py-1 bg-purple-100 text-purple-800 dark:bg-purple-500/20 dark:text-purple-300 rounded font-semibold">
				...
				{#if path.stakeholders && path.stakeholders.length > 0}
					<div class="flex flex-wrap items-center gap-2">
						{#each path.stakeholders as stakeholder}
							<span class="px-2 py-1 bg-amber-100 text-amber-800 dark:bg-amber-500/20 dark:text-amber-300 rounded">
								{stakeholder.str}
				...
				{#if fearedEvents && fearedEvents.length > 0}
					<div class="flex flex-wrap items-center gap-2">
						{#each fearedEvents as fe}
							<span class="px-2 py-1 bg-orange-100 text-orange-800 dark:bg-orange-500/20 dark:text-orange-300 rounded">
								{fe.name}
							</span>
					...
					{#if uniqueAssets.length > 0}
						<div class="flex flex-wrap items-center gap-2">
							{#each uniqueAssets as asset}
								<span class="px-2 py-1 bg-cyan-100 text-cyan-800 dark:bg-cyan-500/20 dark:text-cyan-300 rounded text-xs">
									{asset.str}
								</span>
							{/each}
						</div>
...
<div class="mb-6 bg-surface-50-950 p-3 rounded-lg border border-surface-200-800">
	...
	<div class="flex flex-wrap gap-3 text-xs">
		<span class="px-2 py-1 bg-red-100 text-red-800 dark:bg-red-500/20 dark:text-red-300 rounded font-semibold">{m.riskOrigin()}</span>
		<span class="px-2 py-1 bg-purple-100 text-purple-800 dark:bg-purple-500/20 dark:text-purple-300 rounded font-semibold"
			>{m.targetObjective()}</span
		>
		<span class="px-2 py-1 bg-amber-100 text-amber-800 dark:bg-amber-500/20 dark:text-amber-300 rounded">{m.stakeholder()}</span>
		<span class="px-2 py-1 bg-orange-100 text-orange-800 dark:bg-orange-500/20 dark:text-orange-300 rounded">{m.fearedEvent()}</span>
		<span class="px-2 py-1 bg-cyan-100 text-cyan-800 dark:bg-cyan-500/20 dark:text-cyan-300 rounded">{m.asset()}</span>
	</div>
</div>
