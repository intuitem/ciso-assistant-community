<script lang="ts">
	...
	onMount(async () => {
		options = await fetch('/vulnerabilities/severity').then((r) => r.json());
	});

	async function changeSeverity(newSeverity: string) {
		const endpoint = `/vulnerabilities/${row?.meta?.id}/severity`;
		const requestInit = {
			method: 'PATCH',
			headers: {
...
<ContextMenu.Sub>
	...
	>
		<div class="flex items-center">{m.changeSeverity()}</div>
	</ContextMenu.SubTrigger>
	...
	>
		{#each options as option}
			<ContextMenu.Item
				...
				onclick={async () => await changeSeverity(option.value)}
			>
