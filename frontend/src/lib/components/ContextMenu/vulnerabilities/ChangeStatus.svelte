<script lang="ts">
	...
	onMount(async () => {
		options = await fetch('/vulnerabilities/status').then((r) => r.json());
	});

	async function changeStatus(newStatus: string) {
		const endpoint = `/vulnerabilities/${row?.meta?.id}/status`;
		const requestInit = {
			method: 'PATCH',
			headers: {
		...
		try {
			...
			flash.set({
				type: 'success',
				message: m.successfullyUpdatedObject({ object: m.vulnerability().toLowerCase() })
			});
			handler.invalidate();
		} catch (error) {
			flash.set({
				type: 'error',
				message: m.errorUpdatingObject({ object: m.vulnerability().toLowerCase() })
			});
