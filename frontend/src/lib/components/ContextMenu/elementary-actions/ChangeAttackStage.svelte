<script lang="ts">
	...
	onMount(async () => {
		options = await fetch('/elementary-actions/attack_stage').then((r) => r.json());
	});

	async function changeAttackStage(newStage: string) {
		...
		try {
			...
			flash.set({
				type: 'success',
				message: m.successfullyUpdatedObject({ object: m.elementaryAction().toLowerCase() })
			});
			handler.invalidate();
		} catch (error) {
			flash.set({
				type: 'error',
				message: m.errorUpdatingObject({ object: m.elementaryAction().toLowerCase() })
			});
...
<ContextMenu.Sub>
	...
	>
		<div class="flex items-center">{m.changeAttackStage()}</div>
	</ContextMenu.SubTrigger>
