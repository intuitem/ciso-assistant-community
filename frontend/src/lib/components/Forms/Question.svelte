<script lang="ts">
	...
	import { hasQuestionAnswer, normalizeQuestionGroups } from '$lib/utils/question-groups';
	...
	let visibleQuestions = $derived(
		Object.entries(questions).filter(([, question]) =>
			isQuestionVisible(question, internalAnswers, questions)
		)
	);
	let answeredQuestionCount = $derived(
		visibleQuestions.filter(([urn, question]) =>
			hasQuestionAnswer(
				internalAnswers[urn],
				question.type === 'file' ? (attachments[urn]?.length ?? 0) : 0
			)
		).length
	);

	$effect(() => {
		if (activeGroupIndex >= normalizedGroups.length) activeGroupIndex = 0;
	});
	...
	function toggleSelection(urn: string, optionUrn: string) {
		...
		if (internalAnswers[urn].includes(optionUrn)) {
			internalAnswers[urn] = internalAnswers[urn].filter((val: string) => val !== optionUrn);
		} else {
...
<div>
	...
	<div
		class={hasQuestionGroups
			? 'rounded-xl border border-surface-300-700 bg-surface-100-900/40 p-3 sm:p-4'
			: ''}
	>
		{#if hasQuestionGroups}
			<div class="mb-4 flex items-center justify-between gap-3">
				<div class="min-w-0">
					...
					{/if}
					<p class="flex flex-wrap items-center gap-x-1.5 text-xs text-surface-500">
						{#if questionView === 'grouped'}
							<span>{activeGroupIndex + 1} / {normalizedGroups.length}</span>
				...
				<div class="flex shrink-0 items-center gap-2">
					<button
						...
						aria-pressed={questionView === 'flat'}
						aria-label={questionView === 'grouped' ? m.questionFlatView() : m.questionGroupedView()}
						title={questionView === 'grouped' ? m.questionFlatView() : m.questionGroupedView()}
						onclick={() => {
							questionView = questionView === 'grouped' ? 'flat' : 'grouped';
						}}
					>
						<i class="fa-solid {questionView === 'grouped' ? 'fa-list' : 'fa-layer-group'}"></i>
						<span class="hidden sm:inline"
							>{questionView === 'grouped' ? m.questionFlatView() : m.questionGroupedView()}</span
						>
					</button>
					{#if questionView === 'grouped'}
						<button
						...
						<button
							type="button"
							class="btn-icon preset-tonal"
							disabled={activeGroupIndex === normalizedGroups.length - 1}
							aria-label={m.next()}
							title={m.next()}
							onclick={() => activeGroupIndex++}
						>
		...
		{#each questionGroupsToRender as group (group.order[0])}
			<section
				class={hasQuestionGroups && questionView === 'flat'
			...
			>
				...
				<ul class="control flex flex-col gap-4 whitespace-pre-line">
					{#each group.order as urn}
						{@const question = questions[urn]}
						...
						{#if isQuestionVisible(question, internalAnswers, questions)}
							...
							>
								...
								{:else if question.type === 'unique_choice'}
									...
									{:else}
										<div
											class="flex flex-col gap-1.5 rounded-lg border border-surface-200-800 p-1.5"
										>
								...
								{:else if question.type === 'multiple_choice'}
									<div class="flex flex-col gap-1.5 rounded-lg border border-surface-200-800 p-1.5">
										{#each question.choices as option}
											{@const selected =
												Array.isArray(internalAnswers[urn]) &&
												internalAnswers[urn].includes(option.urn)}
											<button
												type="button"
												name="question"
												{disabled}
									...
									{:else}
										<div>
											<input
												...
												onchange={() => {
													const val =
														questionBuffers[urn] === '' ? null : Number(questionBuffers[urn]);
