	...
	onMount(() => {
		...
		(async () => {
			...
			const disposeChart = mountThemeAwareChart(
				...
				{
					// re-run per init: instance listeners are dropped, and the graphic
					// elements need the live instance to read its pixel size
					onChart: (c: any) => {
						chart = c;
						c.on('click', function (params: any) {
							if (params.value && params.value[5]) {
								goto(`/applied-controls/${params.value[5]}`);
							}
						});
						c.setOption({ graphic: getGraphicElements(c) });
					}
				}
			);
			// graphic coords are pixel-based, so they must be recomputed on resize
			const onResize = () => chart?.setOption({ graphic: getGraphicElements(chart) });
