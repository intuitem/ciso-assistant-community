...
{#if $modalStore[0]}
	...
	<div class="{cBase}">
		...
		<p class="text-surface-600">
			The following scenarios have multiple residual hypotheses. Please select one residual hypothesis per scenario to include it in the action plan.
		</p>
						...
						<div class="space-y-2">
							...
							<div class="grid grid-cols-1 md:grid-cols-2 gap-2">
								{#each scenario.residual_hypotheses as hypothesis}
									<div class="flex items-center space-x-2 p-2 rounded-lg {hypothesis.is_selected ? 'bg-success-100 border border-success-300' : 'bg-surface-100'}">
										<div class="flex-1">
		...
		<footer class="flex justify-end pt-4">
			<button type="button" class="btn variant-filled" onclick={closeModal}>
				Close
			</button>
		</footer>
	</div>
