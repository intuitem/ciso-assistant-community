<script lang="ts">
    ...
    for (let x = 0; x < metrics.data.length; x++)
    {
        ...
        dataE[4] = frameworksMappings[x].assessable_requirements_count - dataE[0] - dataE[1] - dataE[2] - dataE[3]
    }
</script>
...
uuids={metrics.uuids}
seriesNames={['compliant', 'partially compliant', 'non compliant', 'not applicable', 'not assessed']}
