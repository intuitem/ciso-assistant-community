<script lang="ts">
	...
	interface Props {
		...
		/** Purely used for identifying which library is being currently loaded.
		 *
		 * This prop can be omitted when there's only a single `<LibraryUpdateButton/>` component present the page.
		 */
		libraryURN?: string;
	...
	function choicesModal(choices: object[]): void {
		const modalComponent: ModalComponent = {
			...
			props: {
				parent: null,
				formAction: `/loaded-libraries/${loadedLibraryID}?/update`,
				choices: choices,
...
{#if loading.form && loading.library === libraryURN}
...
{:else}
	<span class="hover:text-primary-500">
		<form
			method="post"
			action="/loaded-libraries/{loadedLibraryID}?/update"
			use:enhance={() => {
				loading.form = true;
				loading.library = libraryURN;
