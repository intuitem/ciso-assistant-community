<script lang="ts">
	...
	interface Props {
		entries: FileEntry[];
		evidenceLinkPrefix?: string; // e.g. "/evidences" — prepended for evidence_id links
	...
	const outcomeBadge: Record<BatchOutcome, { label: string; cls: string; icon: string }> = {
		created: { label: 'Created', cls: 'bg-green-100 text-green-800', icon: 'fa-circle-plus' },
		revision_added: {
			label: 'Revision added',
			cls: 'bg-blue-100 text-blue-800',
			icon: 'fa-code-branch'
		},
		replaced: { label: 'Replaced', cls: 'bg-violet-100 text-violet-800', icon: 'fa-rotate' },
		renamed: { label: 'Renamed', cls: 'bg-amber-100 text-amber-800', icon: 'fa-pen' },
		skipped: { label: 'Skipped', cls: 'bg-gray-100 text-gray-700', icon: 'fa-forward' },
		duplicate: {
			label: 'Duplicate',
			cls: 'bg-cyan-100 text-cyan-800',
			icon: 'fa-clone'
		},
		error: { label: 'Error', cls: 'bg-red-100 text-red-800', icon: 'fa-triangle-exclamation' }
	};
</script>
	...
	<div class="border rounded-lg overflow-hidden">
		<table class="w-full text-sm">
			<thead class="bg-gray-50 text-left">
				<tr>
					<th class="px-3 py-2 font-medium">Path / Name</th>
