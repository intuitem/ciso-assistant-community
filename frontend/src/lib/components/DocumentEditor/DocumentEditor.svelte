<script lang="ts">
	...
	async function switchLocale(locale: string) {
		...
		const res = await fetch(`${proxyUrl}?_action=documents-by-locale&locale=${locale}`);
		if (!res.ok) return;
		...
		confirmAndDelete(m.deleteDocumentTitle(), m.deleteDocumentConfirmation(), async () => {
			...
			if (res.ok) {
				...
				if (availableLocales.length > 0) {
					...
					// No translations left — go back to the parent
...
<div class="flex flex-col h-full -m-8 bg-surface-50-950 min-h-screen">
	...
	>
		<div class="flex items-center gap-3">
			<a href={backHref} class="btn btn-sm preset-tonal-surface" title={m.backToPolicy()}>
