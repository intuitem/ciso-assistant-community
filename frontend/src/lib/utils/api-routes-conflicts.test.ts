...
const STANDARD_IMPORT_REGEX = /^import\s+{\s*(([a-zA-Z0-9_]+\s+,\s+)*[a-zA-Z0-9_]+)\s*}\s+from\s+('.*?'|".*?")[\s;]*$/;
    ...
    // Replace every svelte component imports by a fake dummy component definition.
		...
		if (match) {
			...
			if (IMPORT_PATH_TO_REPLACE.has(importedLib)) {
				const dummyObjMembers = importedNames.map((importedName) => `${importedName}: null`).join(", ");
...
function getDirAndFileNames(path: string): [Set<string>, Set<string>] {
  ...
  const directoryNames = dirEntries.filter(
    (dirEntry) => dirEntry.isDirectory()
  ).map((dirEntry) => dirEntry.name);

  const fileNames = dirEntries.filter(
 ...
 */
function checkRouteConflict(fileNameSet: Set<string>, routePath: string, conflictRoutePath: string) {
...
describe('checkSveltekitRouteConflicts', () => {
  it('checks route conflicts', () => {
    ...
    for (const route of urlmodelRoutes) {
      ...
      if (dirNameSet.has("[id=uuid]")) {
        ...
        for (const fieldRoute of fieldRoutes) {
          ...
          const conflictRoute = path.join(INTERNAL_ROUTE_PATH, "[model=urlmodel]/[id=uuid]/[field=fields]");
    ...
    const thirdPartyUrlmodelRoutes = thirdPartyRouteList.filter(filterSpecialRoutes(thirdPartyUrlModelMatch));
    ...
    for (const route of thirdPartyUrlmodelRoutes) {
      ...
      if (dirNameSet.has("[id=uuid]")) {
        ...
        const conflictRoute = path.join(INTERNAL_ROUTE_PATH, "[model=thirdparty_urlmodels]/[id=uuid]");
        checkRouteConflict(fileNameSet, detailedRoutePath, conflictRoute)
      }
    }
  });
