...
from ciso_assistant.settings import (EMAIL_HOST_PASSWORD_RESCUE,
                                     EMAIL_HOST_RESCUE, EMAIL_HOST_USER_RESCUE,
                                     EMAIL_PORT_RESCUE, EMAIL_USE_TLS_RESCUE,
                                     CISO_URL)
from core.base_models import *
from core.utils import BUILTIN_ROLE_CODENAMES, BUILTIN_USERGROUP_CODENAMES
...
class UserManager(BaseUserManager):
    ...
    def _create_user(self, email, password, **extra_fields):
        ...
        if password:
            ...
            try:
                user.mailing(email_template_name="registration/first_connexion_email.html", subject=_("Welcome to CISO Assistant!"))
...
class RoleAssignment(models.Model):
    ...
    @staticmethod
    def get_accessible_object_ids(folder: Folder, user: User, object_type: models.Model) -> Tuple[list[str], list[str], list[str]]:
        ...
        for ra in [x for x in RoleAssignment.get_role_assignments(user) if ref_permission in x.role.permissions.all()]:
            ...
            for my_folder in perimeter & set(ra.perimeter_folders.all()):
                ...
                for p in [p for p in permissions if p in ra_permissions]:
                    ...
                    for obj in [x for x in all_objects if folder_for_object[x] in target_folders]:
                        if p == permissions[0]:
                            permissions_per_object_id[obj.id].add(p)
                            continue
                        if hasattr(obj, "builtin") and obj.builtin:
                            continue
                        # builtin objects and objects from intuitem's referential cannot be edited
                        object_urn = getattr(obj, "urn", "")
                        if object_urn and p == permissions[1]:
                            continue
                        is_deletable = hasattr(obj, "is_deletable") and obj.is_deletable() or not hasattr(obj, "is_deletable")
                        if not is_deletable and p == permissions[2]:
                            continue
                        permissions_per_object_id[obj.id].add(p)
        ...
        if hasattr(object_type, "is_published"):
            for my_folder in folders_with_local_view:
                ...
                for obj in [x for x in all_objects if folder_for_object[x] in target_folders and x.is_published]:
                    permissions_per_object_id[obj.id].add(permissions[0])
